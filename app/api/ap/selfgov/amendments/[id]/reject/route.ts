...
import { rejectDraftedAmendment } from "@/lib/ap-intake/selfgov/service";
...
export async function POST(req: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const result = await rejectDraftedAmendment({
      firmId,
      firmClientId: String(body.firm_client_id),
      engagementId: String(body.engagement_id),
      amendmentId: id,
      rejectedByUserId: String(body.rejected_by_user_id ?? auth.userId),
