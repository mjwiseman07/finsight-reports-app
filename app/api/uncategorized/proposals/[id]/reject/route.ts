...
import { rejectProposal } from "@/lib/learning/proposal-service";
...
export async function POST(req: Request, context: RouteContext) {
  ...
  if (!body?.reject_reason) {
    return NextResponse.json({ error: "missing_reject_reason" }, { status: 400 });
  }
  ...
  try {
    await rejectProposal(id, access.userId, body);
    return NextResponse.json({ status: "rejected", proposal_id: id });
  } catch (err) {
