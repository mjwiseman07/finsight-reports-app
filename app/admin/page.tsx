  ...
  return (
    <div className="min-h-screen bg-[#111112] text-[#ECEBE7]">
      ...
      <main className="mx-auto max-w-7xl px-6 py-10 sm:px-8">
        {overview.admin && (
          <div className="sticky top-3 z-40 mt-5 grid gap-3 rounded-3xl border border-[#B85C5C]/40 bg-[#B85C5C]/15 p-4 text-sm font-bold text-[#F0BFBF] shadow-2xl shadow-black/40 backdrop-blur-xl md:grid-cols-4">
            <div>
              <span className="block text-xs uppercase tracking-[0.18em] text-[#F0BFBF]/80">SUPER ADMIN MODE</span>
              <span>{overview.admin.email}</span>
            </div>
            <div>
              <span className="block text-xs uppercase tracking-[0.14em] text-[#F0BFBF]/80">Company Persona</span>
              <span>{selectedCompany?.personaMode || "Configured during onboarding"}</span>
            </div>
            <div>
              <span className="block text-xs uppercase tracking-[0.14em] text-[#F0BFBF]/80">Company Package</span>
              <span>{selectedCompany?.packageLevel || "Configured during onboarding"}</span>
            </div>
            <div>
              <span className="block text-xs uppercase tracking-[0.14em] text-[#F0BFBF]/80">Current Test Company</span>
        ...
        {error && (
          <div className="mt-6 rounded-3xl border border-[#B85C5C]/30 bg-[#B85C5C]/15 p-6 text-sm font-bold text-[#F0BFBF]">
            {error}
          </div>
        )}

        {message && (
          <div className="mt-6 rounded-3xl border border-[#6DAA45]/30 bg-[#6DAA45]/15 p-6 text-sm font-bold text-[#B5E28A]">
            {message}
          </div>
        )}
        ...
        {!isLoading && !error && overview.admin && (
          <section className="mt-8 grid gap-6">
            <section className="rounded-[2rem] border border-[#C9A961]/20 bg-[#1A1A1C]/85 p-6 shadow-2xl shadow-black/40">
              <p className={`${headingFont} text-sm font-black uppercase tracking-[0.22em] text-[#C9A961]`}>Super Admin Dashboard</p>
              <h1 className={`${headingFont} mt-3 text-4xl font-black tracking-[-0.04em] text-[#ECEBE7] md:text-5xl`}>Company Testing Center</h1>
              ...
              <div className="mt-5">
                <Link
                  href="/admin/refunds"
                  className={`inline-flex items-center gap-2 rounded-full border border-[#C9A961]/25 bg-[#111112] px-5 py-3 text-sm font-black text-[#ECEBE7] transition hover:border-[#C9A961]/60 hover:bg-[#C9A961]/10 ${focusRing()}`}
                >
                  Refund Queue
                  {pendingRefundCount > 0 ? (
                    <span className="rounded-full bg-[#C9A961] px-2.5 py-0.5 text-xs font-black text-[#111112]">
                      {pendingRefundCount}
                    </span>
                  ) : null}
                </Link>
                <p className="mt-2 text-xs font-bold text-[#7A7974]">
                  Governed by Advisacor Refund Policy v1 · Path B founder review
                </p>
              </div>
              <div className="mt-5 flex flex-wrap gap-2">
                {(overview.screens || superAdminScreens).map((screen) => (
                  <a key={screen} href={`#${screen.toLowerCase().replace(/[^a-z0-9]+/g, "-")}`} className={`rounded-full bg-[#C9A961]/10 px-3 py-1 text-xs font-black text-[#A29E93] transition hover:bg-[#C9A961]/15 hover:text-[#ECEBE7] ${focusRing()}`}>
                    {screen}
                  </a>
                ))}
              </div>
            ...
            <section id="system-health" className="grid gap-5 md:grid-cols-2 xl:grid-cols-4">
              {Object.entries(overview.system_health || {}).map(([key, value]) => (
                <div key={key} className="rounded-3xl border border-[#C9A961]/20 bg-[#1A1A1C]/85 p-5 shadow-2xl shadow-black/40">
                  <p className="text-xs font-black uppercase tracking-[0.16em] text-[#7A7974]">{key.replace(/_/g, " ")}</p>
                  <p className={`mt-3 text-2xl font-black ${value ? "text-[#B5E28A]" : "text-[#F0BFBF]"}`}>
            ...
            <section className="grid gap-6 xl:grid-cols-[1fr_0.8fr]">
              <Panel id="customer-experience-qa" title="Customer Experience QA">
                <div className="rounded-3xl border border-[#6DAA45]/25 bg-[#6DAA45]/10 p-5">
                  <p className={`${headingFont} text-sm font-black uppercase tracking-[0.18em] text-[#B5E28A]`}>Customer Test Account</p>
                  <h2 className={`${headingFont} mt-3 text-2xl font-black text-[#ECEBE7]`}>demo@advisacor.com</h2>
                  <p className="mt-3 text-sm leading-6 text-[#B5E28A]/85">
                  ...
                  <div className="mt-4 grid gap-2 text-sm font-bold text-[#B5E28A]/80">
                    <p>Role: Customer</p>
                  ...
                  <button
                    ...
                    className={`mt-5 rounded-full bg-[#C9A961] px-5 py-3 text-sm font-black text-[#111112] transition hover:bg-[#DFC084] disabled:cursor-not-allowed disabled:opacity-60 ${focusRing()}`}
                  >
                    View As Customer
                  </button>
                </div>
              ...
              <Panel id="company-testing-center" title="Company Testing Center">
                <div className="mb-6 rounded-3xl border border-[#C9A961]/25 bg-[#C9A961]/10 p-5">
                  <p className="text-sm font-black text-[#ECEBE7]">Test Journey Launcher</p>
                  ...
                  <div className="mt-4 grid gap-3 md:grid-cols-2">
                    <button
                      ...
                      className={`rounded-2xl border border-[#C9A961]/20 bg-[#111112] px-4 py-3 text-left text-sm font-black text-[#ECEBE7] transition hover:border-[#C9A961]/60 disabled:cursor-not-allowed disabled:opacity-60 ${focusRing()}`}
                    >
                      <span className="block">Start My Own Company</span>
                      <span className="mt-1 block text-xs font-bold leading-5 text-[#7A7974]">Test owner/internal team onboarding from Step 1.</span>
                    </button>
                    <button
                      type="button"
                    ...
                    >
                      ...
                      <span className="mt-1 block text-xs font-bold leading-5 text-[#7A7974]">Test solo or firm practice setup plus first client.</span>
                    </button>
                    <button
                      type="button"
                    ...
                    >
                      ...
                      <span className="mt-1 block text-xs font-bold leading-5 text-[#7A7974]">Test advisory firm setup plus client executive reporting.</span>
                    </button>
                    <button
                      type="button"
                    ...
                    >
                      ...
                      <span className="mt-1 block text-xs font-bold leading-5 text-[#7A7974]">Open the selected demo company as a test user session.</span>
                    </button>
                  </div>
                </div>
                    ...
                    <button
                      ...
                      className={`rounded-3xl border p-4 text-left transition ${
                        selectedCompany?.id === company.id
                          ...
                          : "border-[#C9A961]/20 bg-[#1A1A1C]/85 hover:border-[#C9A961]/40"
                      } ${focusRing()}`}
                    >
                      <p className="text-sm font-black text-[#ECEBE7]">{company.name}</p>
                      <p className="mt-1 text-xs font-bold uppercase tracking-[0.14em] text-[#7A7974]">{company.industry || company.status}</p>
                      <div className="mt-4 grid gap-2 text-xs font-bold text-[#A29E93]">
                        <p>Admin user: <span className="text-[#ECEBE7]">{company.companyAdmin || "Demo admin"}</span></p>
                        <p>Industry type: <span className="text-[#ECEBE7]">{company.industryType || "Other"}</span></p>
                        <p>Primary persona: <span className="text-[#ECEBE7]">{company.personaMode}</span></p>
                        <p>Package: <span className="text-[#ECEBE7]">{company.packageLevel}</span></p>
                      </div>
                      <div className="mt-4 flex flex-wrap gap-2">
                        <span className="rounded-full border border-[#6DAA45]/25 bg-[#6DAA45]/15 px-3 py-1 text-xs font-black text-[#B5E28A]">{company.setupStatus || "Configured"}</span>
                        <span className="rounded-full bg-[#C9A961]/10 px-3 py-1 text-xs font-black text-[#A29E93]">{company.packageLevel}</span>
                        <span className="rounded-full bg-[#C9A961]/10 px-3 py-1 text-xs font-black text-[#A29E93]">{company.personaMode}</span>
                      </div>
                    </button>
                  ))}
                </div>
                ...
                <details className="mt-4 rounded-2xl border border-[#C9A961]/20 bg-[#1A1A1C]/85 p-4">
                  ...
                  <button
                    ...
                    disabled={isRunningAction || !selectedCompany}
                    className={`mt-4 rounded-full bg-[#B85C5C] px-5 py-3 text-sm font-black text-[#111112] transition hover:bg-[#C97070] disabled:cursor-not-allowed disabled:opacity-60 ${focusRing()}`}
                  >
                ...
                {selectedCompany?.demoData && (
                  <div className="mt-6 rounded-3xl border border-[#C9A961]/20 bg-[#1A1A1C]/85 p-5">
                    <p className="text-sm font-black text-[#ECEBE7]">Configured Demo Data</p>
                    <div className="mt-4 grid gap-3 md:grid-cols-2">
                      {Object.entries(selectedCompany.demoData).map(([section, values]) => (
                        <div key={section} className="rounded-2xl border border-[#C9A961]/20 bg-[#111112] p-4">
                          <p className="text-xs font-black uppercase tracking-[0.14em] text-[#DFC084]">{section.replace(/([A-Z])/g, " $1")}</p>
                          <div className="mt-3 grid gap-2">
                            {Object.entries(values).slice(0, 4).map(([key, value]) => (
                              <div key={key} className="flex items-center justify-between gap-3 text-xs">
                                <span className="capitalize text-[#7A7974]">{key.replace(/([A-Z])/g, " $1")}</span>
                                <span className="font-black text-[#ECEBE7]">
              ...
              <Panel id="user-management" title="User Management">
                <div className="rounded-3xl border border-[#C9A961]/20 bg-[#1A1A1C]/85 p-5">
                  <p className="text-sm font-black text-[#ECEBE7]">Super Admin User</p>
                  <p className="mt-2 text-sm font-bold text-[#A29E93]">{overview.admin.email}</p>
                  <p className="mt-1 text-xs font-bold uppercase tracking-[0.14em] text-[#7A7974]">{overview.admin.role}</p>
                </div>
                <div className="mt-4 rounded-3xl border border-[#C9A961]/20 bg-[#1A1A1C]/85 p-5">
                  <p className="text-sm font-black text-[#ECEBE7]">Selected Company Users</p>
                  <div className="mt-3 grid gap-3">
                    {(selectedCompany?.users || []).map((user, index) => (
                      <div key={`${user.email}-${user.role}-${user.status}-${index}`} className="rounded-2xl border border-[#C9A961]/20 bg-[#111112] p-4">
                        <p className="text-sm font-black text-[#ECEBE7]">{user.email}</p>
                    ...
                    {(selectedCompany?.users || []).length === 0 && (
                      <p className="rounded-2xl bg-[#111112] px-4 py-3 text-sm font-bold text-[#A29E93]">No demo users configured.</p>
                    )}
                  </div>
                </div>
                <button
                  ...
                  disabled={isRunningAction}
                  className={`mt-4 rounded-full border border-[#C9A961]/20 bg-[#1A1A1C]/85 px-5 py-3 text-sm font-black text-[#ECEBE7] transition hover:border-[#C9A961]/60 disabled:cursor-not-allowed disabled:opacity-60 ${focusRing()}`}
                >
                  View Company Users
                </button>
              </Panel>
            ...
            <Panel id="integration-validation" title="Integration Validation">
              <div className="rounded-3xl border border-[#C9A961]/25 bg-[#C9A961]/10 p-5">
                <p className="text-sm font-black text-[#ECEBE7]">Unified ERP Data Engine</p>
                <p className="mt-2 text-sm leading-6 text-[#A29E93]">
                ...
                <div className="mt-5 grid gap-4">
                  <div className={`rounded-3xl border p-5 ${
                    integrationValidation.ok
                      ? "border-[#6DAA45]/30 bg-[#6DAA45]/15"
                      : "border-[#B85C5C]/30 bg-[#B85C5C]/15"
                  }`}
                  >
                    <p className={`text-sm font-black ${integrationValidation.ok ? "text-[#B5E28A]" : "text-[#F0BFBF]"}`}>
                      {integrationValidation.ok ? "All integration checks passed." : "Integration validation requires attention."}
                    </p>
                    <p className="mt-2 text-xs font-bold text-[#A29E93]">
                    ...
                    <p className="mt-1 text-xs font-bold text-[#7A7974]">Checked at {integrationValidation.checkedAt}</p>
                  </div>

                  <div className="grid gap-3 md:grid-cols-2">
                    {integrationValidation.checks.map((check) => (
                      <div key={check.name} className="rounded-2xl border border-[#C9A961]/20 bg-[#1A1A1C]/85 p-4">
                        <p className={`text-sm font-black ${check.status === "pass" ? "text-[#B5E28A]" : "text-[#F0BFBF]"}`}>
                          {check.status === "pass" ? "PASS" : "FAIL"} | {check.name}
                        </p>
                        <p className="mt-2 text-xs leading-5 text-[#7A7974]">{check.detail}</p>
                      </div>
                    ))}
                  </div>
                </div>
            ...
            <section className="grid gap-6 xl:grid-cols-2">
              {adminWorkspaceCards.map((card) => (
                <Panel key={card.title} id={card.title.toLowerCase().replace(/[^a-z0-9]+/g, "-")} title={card.title}>
                  <p className="text-sm leading-6 text-[#A29E93]">{card.description}</p>
                  <div className="mt-4 rounded-2xl border border-[#C9A961]/20 bg-[#111112] px-4 py-3 text-xs font-bold text-[#A29E93]">
            ...
            <Panel id="support-center" title="Support Center">
              ...
              <div className="mt-5 flex flex-col gap-3 md:flex-row">
                <input
                  ...
                  className="min-w-0 flex-1 rounded-2xl border border-[#C9A961]/20 bg-[#111112] px-4 py-3 text-sm font-bold text-[#ECEBE7] outline-none focus:ring-2 focus:ring-[#C9A961]/50"
                />
                <button
                  type="button"
              ...
              {supportMessage && <p className="mt-4 rounded-2xl border border-[#6DAA45]/30 bg-[#6DAA45]/15 p-4 text-sm font-bold text-[#B5E28A]">{supportMessage}</p>}

              <div className="mt-5 grid gap-4">
                {supportTickets.length === 0 && (
                  <p className="rounded-2xl border border-[#C9A961]/20 bg-[#1A1A1C]/85 px-4 py-3 text-sm font-bold text-[#A29E93]">No support tickets found.</p>
                )}
                {supportTickets.map((ticket) => (
                  <article key={ticket.id} className="rounded-3xl border border-[#C9A961]/20 bg-[#1A1A1C]/85 p-5">
                    <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                      <div>
                        <p className="text-sm font-black text-[#ECEBE7]">Ticket #{ticket.ticketNumber}: {ticket.subject}</p>
                        <p className="mt-1 text-xs font-bold uppercase tracking-[0.14em] text-[#7A7974]">
                          {ticket.type} | {ticket.category} | {ticket.priority}
                        </p>
                        <p className="mt-3 text-sm leading-6 text-[#A29E93]">{ticket.description}</p>
                        <p className="mt-3 text-xs font-bold text-[#7A7974]">
                      ...
                      <div className="grid min-w-64 gap-3">
                        <select
                          ...
                          className="rounded-2xl border border-[#C9A961]/20 bg-[#111112] px-4 py-3 text-sm font-bold text-[#ECEBE7]"
                        >
                    ...
                    <textarea
                      ...
                      rows={3}
                      className="mt-4 w-full rounded-2xl border border-[#C9A961]/20 bg-[#111112] px-4 py-3 text-sm font-bold text-[#ECEBE7] outline-none focus:ring-2 focus:ring-[#C9A961]/50"
                    />
                  </article>
                ))}
              </div>
            ...
            <section className="grid gap-6 xl:grid-cols-2">
              <Panel id="job-queue" title="Job Queue">
                <div className="mb-4 rounded-2xl border border-[#6DAA45]/25 bg-[#6DAA45]/10 px-4 py-3">
                  <p className="text-sm font-black text-[#B5E28A]">Job Storage: Configured</p>
                  <p className="mt-1 text-xs leading-5 text-[#B5E28A]/80">
                  ...
                  {overview.job_storage?.production_setup_missing && (
                    <p className="mt-2 text-xs leading-5 text-[#DFC084]">
                ...
                <div className="grid gap-3">
                  {(overview.jobs || []).length === 0 && (
                    <p className="rounded-2xl bg-[#111112] px-4 py-3 text-sm font-bold text-[#A29E93]">No background jobs found yet.</p>
                  )}
                  {(overview.jobs || []).slice(0, 10).map((job) => (
                    <div key={job.id} className="rounded-2xl bg-[#111112] p-4">
                      <p className="text-sm font-black text-[#ECEBE7]">{job.job_type || "Background Job"}</p>
                      <p className="mt-1 text-xs font-bold text-[#7A7974]">{job.status || "unknown"}</p>
                    </div>
                  ))}
                </div>
              </Panel>
              ...
              <Panel id="audit-logs" title="Audit Logs">
                <div className="grid gap-3">
                  {(overview.audit_logs || []).length === 0 && (
                    <p className="rounded-2xl bg-[#111112] px-4 py-3 text-sm font-bold text-[#A29E93]">No audit logs found yet.</p>
                  )}
                  {(overview.audit_logs || []).slice(0, 10).map((log) => (
                    <div key={log.id} className="rounded-2xl bg-[#111112] p-4">
                      <p className="text-sm font-black text-[#ECEBE7]">{log.event_type || "audit_event"}</p>
                      <p className="mt-1 text-xs font-bold text-[#7A7974]">{log.created_at || "No timestamp"}</p>
                    </div>
                  ))}
                </div>
              </Panel>
...
function Panel({ id, title, children }: { id: string; title: string; children: ReactNode }) {
  return (
    <section id={id} className="rounded-[2rem] border border-[#C9A961]/20 bg-[#1A1A1C]/85 p-6 shadow-2xl shadow-black/40">
      <p className={`${headingFont} text-sm font-black uppercase tracking-[0.18em] text-[#C9A961]`}>{title}</p>
