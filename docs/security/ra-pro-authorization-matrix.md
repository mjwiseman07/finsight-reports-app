...
**Checkout bootstrap (DB):** `bootstrap_checkout_firm_workspace` / `bootstrap_checkout_company_workspace` create firm/company + canonical relationship atomically (service_role only). Unlinked firm identity = exact `owner_user_id` ∪ **active**-membership firm set (conflict if >1). Company identity = exact **active** ownership-class `company_users` set. Any non-active ownership/membership row (`status IS DISTINCT FROM 'active'`) → `bootstrap_checkout_ownership_revoked` (no reactivate, no replacement create, no mutation)—firm and company aligned. No `ORDER BY … LIMIT 1`. `/api/company/onboarding` uses the same company RPC. Capacity lock/isolation → sanitized HTTP 503 `workspace_bootstrap_retryable`; ownership conflict/revoked → 409 before Stripe customer/session create.
