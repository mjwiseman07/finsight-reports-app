...
1. **Deterministic assembler** — `scripts/migration-remediation/assemble-option-d-replay.js`  
   - Foundations baseline + phase1 recovered + **recovered required originals** (tables + function sources, post-prefix) + post-phase1 local files  
   ...
   - Recovered required CREATE/RENAME/FUNCTION bodies are **originals** (no substitution) — see `evidence/option-d-required-creates/` and `evidence/option-d-required-functions/`
...
4. **Gates**
   - **Option D candidate gate** (`audit-option-d-replay-gate.js`) — `mergeReady` means `candidateReplayStaticReady` only (fixture scan **and** required table **and** function CREATE dependencies resolved). It is **not** overall PR merge and **not** runtime PASS. `prMergeReady` / `runtimeReady` stay false. Static PASS is not a local replay.
...
|-------|--------------------------|
| `candidateReplay` | Static PASS once required table **and** function CREATEs are in-set; runtime PASS only after a later fresh-DB apply |
