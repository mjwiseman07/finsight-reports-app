...
async function verifyBackgroundJobs() {
  ...
  const { data, error } = await supabaseAdmin.from("background_jobs").select("*").limit(5);

  if (error) {
    console.log("background_jobs table: not operational");
