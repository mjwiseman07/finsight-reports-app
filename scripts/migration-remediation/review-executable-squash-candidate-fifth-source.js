#!/usr/bin/env node
/**
 * Fifth independent source review — users-UPDATE + function-hardening ESC candidate.
...
const COMMIT = process.env.ESC_REVIEW_COMMIT || '1a4c8182b68a2506f85140970073558fb1b1ebb2';
...
const PRIOR_SEAL = 'c5c360d8325e2cbfa474d97ea0d33e0f2449ab89820770146def8c4c13da5a37';
...
const OUT_AUTO = path.join(
  ROOT,
  'docs/migration-remediation/evidence/executable-squash-candidate-source-review-fifth-2026-09-07.auto.md'
);
...
function main() {
  ...
  if (users.staleUpdatePolicy) {
    add(
      'P2',
      'USERS_STALE_UPDATE_RLS_POLICY',
      '10010',
      'FOR UPDATE own-row policy remains after table UPDATE revoke (dead policy; not a write path)'
    );
  }
  ...
  const report = {
    ...
    usersSecurity: { ...users, browserUsersUpdate },
    ...
    JSON.stringify(
      {
        ...
        candidateIdentical,
        usersUpdateRevoked: users.updateRevokedAuth && users.noTableUpdateGrantAuth,
      },
      null,
      2
    )
