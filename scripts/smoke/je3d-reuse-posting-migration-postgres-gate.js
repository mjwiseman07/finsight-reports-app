// @ts-nocheck
/**
 * Compile gate: apply repaired 20260826043000 migration inside a rolled-back transaction.
 ...
 *   node scripts/smoke/je3d-reuse-posting-migration-postgres-gate.js
 */
...
async function main() {
  ...
  try {
    ...
    if (verify.rows.length !== 2) {
      throw new Error(`expected 2 functions, got ${verify.rows.length}`);
    }
    if (!verify.rows.every((r) => r.prosecdef)) {
      throw new Error("expected SECURITY DEFINER on both functions");
    }
