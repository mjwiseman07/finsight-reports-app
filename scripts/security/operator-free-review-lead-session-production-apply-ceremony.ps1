...
<#
.SYNOPSIS
  Operator-assisted production APPLY ceremony for Free Review lead sessions (FRLS).
  ...
  $evidence = [ordered]@{
    ...
    credential_redaction_confirmation = [ordered]@{
      ...
      harness_env_absent_after = [string]::IsNullOrWhiteSpace(
        [Environment]::GetEnvironmentVariable("FRLS_CEREMONY_ALLOW_SYNTHETIC_URL", "Process")
      )
    }
    cleanup = [ordered]@{
