...
<#
.SYNOPSIS
  External supervisor for the visible FRLS lead-session ceremony.
...
.DESCRIPTION
  ...
  EvidenceOutDir/material-supervisor-*/enter-visible-free-review-lead-session-ceremony.ps1,
  ...
  Residual: if THIS supervisor is hard-killed, no FRLS_LEAD_SESSION_EVIDENCE_V1 can
  ...
  Never sets FREE_REVIEW_LEAD_SESSION_APPLY_DATABASE_URL. Synthetic -TestStubScript only.
#>
[CmdletBinding()]
param(
...
# Fixed type name so callers always use [FrlsVisible.JobApi]::Method
if (-not ("FrlsVisible.JobApi" -as [type])) {
  Add-Type -TypeDefinition @"
using System;
using System.Text;
using System.Runtime.InteropServices;

namespace FrlsVisible {
...
function New-KillOnCloseJob {
  $h = [FrlsVisible.JobApi]::CreateJobObject([IntPtr]::Zero, $null)
  if ($h -eq [IntPtr]::Zero) {
    throw "CreateJobObject failed err=$([Runtime.InteropServices.Marshal]::GetLastWin32Error())"
  }
  $info = New-Object "FrlsVisible.JobApi+JOBOBJECT_EXTENDED_LIMIT_INFORMATION"
  $basic = $info.BasicLimitInformation
  # ONLY kill-on-close — never BREAKAWAY / silent breakaway flags.
  $basic.LimitFlags = [FrlsVisible.JobApi]::JOB_OBJECT_LIMIT_KILL_ON_JOB_CLOSE
  ...
  try {
    ...
    $ok = [FrlsVisible.JobApi]::SetInformationJobObject(
      $h,
      [FrlsVisible.JobApi]::JobObjectExtendedLimitInformation,
      $ptr,
      [uint32]$size
    )
    if (-not $ok) {
    ...
    try { [void][FrlsVisible.JobApi]::CloseHandle($h) } catch {}
    throw
  } finally {
...
function Start-SuspendedInJob {
  ...
  $si = New-Object "FrlsVisible.JobApi+STARTUPINFO"
  $si.cb = [uint32][Runtime.InteropServices.Marshal]::SizeOf($si)
  $pi = New-Object "FrlsVisible.JobApi+PROCESS_INFORMATION"
  $cmd = New-Object System.Text.StringBuilder
  [void]$cmd.Append($CommandLine)
  $flags = [FrlsVisible.JobApi]::CREATE_SUSPENDED
  $ok = [FrlsVisible.JobApi]::CreateProcess(
    $Exe,
    $cmd,
    [IntPtr]::Zero,
    [IntPtr]::Zero,
  ...
  try {
    ...
    $assigned = [FrlsVisible.JobApi]::AssignProcessToJobObject($Job, $pi.hProcess)
    if (-not $assigned) {
      throw "AssignProcessToJobObject failed err=$([Runtime.InteropServices.Marshal]::GetLastWin32Error())"
    }
    $resume = [FrlsVisible.JobApi]::ResumeThread($pi.hThread)
    ...
    try { [void][FrlsVisible.JobApi]::TerminateJobObject($Job, 1) } catch {}
    try { [void][FrlsVisible.JobApi]::CloseHandle($pi.hThread) } catch {}
    try { [void][FrlsVisible.JobApi]::CloseHandle($pi.hProcess) } catch {}
    throw
  }
  try { [void][FrlsVisible.JobApi]::CloseHandle($pi.hThread) } catch {}
...
function Close-JobAndConfirm {
  ...
  if ($script:JobHandle -ne [IntPtr]::Zero) {
    try {
      [void][FrlsVisible.JobApi]::TerminateJobObject($script:JobHandle, 1)
    } catch {}
    try {
      [void][FrlsVisible.JobApi]::CloseHandle($script:JobHandle)
      $jobClosed = $true
    } catch {}
    $script:JobHandle = [IntPtr]::Zero
  }
...
function Write-SupervisorEvidence([hashtable]$Object) {
  ...
  [Console]::Out.WriteLine("FRLS_LEAD_SESSION_EVIDENCE_V1:" + (ConvertTo-Base64Url -Bytes $bytes))
  if ($EvidenceOutDir) {
...
try {
  ...
  $entryDest = Join-Path $script:SupervisorMaterialRoot "enter-visible-free-review-lead-session-ceremony.ps1"

  # Trust root: materialize entry from TIP blob only (never execute worktree entry as authority).
  try {
    [void](Assert-BlobSeal -Commit $tip -Rel "scripts/security/enter-visible-free-review-lead-session-ceremony.ps1" -Seal $ve -Dest $entryDest -WorkDir $RepoRoot)
  } catch {
  ...
  if ($CeremonyKind -eq "apply" -and [string]::IsNullOrWhiteSpace($TestStubScript)) {
