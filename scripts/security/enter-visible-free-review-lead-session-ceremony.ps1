...
.DESCRIPTION
  ...
  This entry:
    ...
    - never sets FREE_REVIEW_LEAD_SESSION_APPLY_DATABASE_URL / never completes credential input
...
try {
  ...
  $launcherDest = Join-Path $script:materialRoot "launch-visible-free-review-lead-session-ceremony.ps1"
  $ceremonyLeaf = [IO.Path]::GetFileName($ceremonyRel)
  $ceremonyDest = Join-Path $script:materialRoot $ceremonyLeaf

  $launcherSeal = $null
  try {
    $launcherSeal = Assert-BlobSeal -Freeze $freeze -Rel "scripts/security/launch-visible-free-review-lead-session-ceremony.ps1" -Seal $vl -Dest $launcherDest -WorkDir $RepoRoot
  } catch {
