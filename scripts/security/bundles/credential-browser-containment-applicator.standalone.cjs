...
var require_credential_browser_containment_constants = __commonJS({
  "scripts/security/credential-browser-containment-constants.js"(exports2, module2) {
    ...
    var EXPECTED_AUTH_SEALS_DIGEST = "5db0f72335e4d4d2d7ffe5e7b5df9431ccdce26c993c6c67fae1a780298bd52e";
...
var require_credential_browser_containment_apply_core = __commonJS({
  "scripts/security/credential-browser-containment-apply-core.js"(exports2, module2) {
    ...
    function redactString(input) {
      ...
      s = s.replace(/CONTAINMENT_APPLY_DATABASE_URL\s*[:=]\s*[^\s)'"`]+/gi, `${DATABASE_URL_ENV2}=***`);
      s = s.replace(/\/\/([^:@\s/'"]+):([^@\s/'"]+)@/g, "//***:***@");
  ...
  `);
    ...
    function assertPreChangeMatch(probe) {
      if (!probe.auth_ac_tok || !probe.auth_qb_tok || !probe.auth_ac_sel || !probe.anon_ac_sel || !probe.residual_select_policy || !probe.qb_browser_policy || !probe.ac_rls || !probe.qb_rls || !probe.view_has_access_token || !probe.view_has_refresh_token || Number(probe.view_dependents) !== 0) {
        throw new Error(
  ...
  `);
      const r = rows[0];
      if (r.auth_sel || r.auth_tok || r.auth_view || !r.svc_upd || !r.svc_view || r.residual_select || Number(r.non_service_policies) !== 0 || r.view_tok) {
    ...
    async function reconcileAfterIndeterminate(databaseUrl, inputs, packed, priorManifest, env = process.env) {
      ...
      try {
        await withClient(databaseUrl, async (client) => {
          ...
          if (result.statement_match && result.containment_ok && result.prior_manifest_unchanged !== false) {
