...
// scripts/security/ra-pro-accounting-automation-prior-dry-run-gates.js
var require_ra_pro_accounting_automation_prior_dry_run_gates = __commonJS({
  "scripts/security/ra-pro-accounting-automation-prior-dry-run-gates.js"(exports2, module2) {
    "use strict";
    var { loadAndVerifyGitBlob } = require_git_blob_authority();
    var PROTOCOL = "RA_PRO_ACCOUNTING_AUTOMATION_PRIOR_DRY_RUN_EVIDENCE_V1";
    var EVIDENCE_PATH = "docs/security/ra-pro-accounting-automation-apply/RA_PRO_ACCOUNTING_AUTOMATION_PRIOR_DRY_RUN_EVIDENCE_V1.json";
    var SOURCE_COMMIT = "195c76060f0fc8aa7b81dc4718d7357c3be8eba6";
    var EVIDENCE_OID = "76a75e41c80e1692670ede5d70cec963a1986f92";
    var EVIDENCE_SHA256 = "f89c3e701703d199f56577a65ae6f28b5ba120be45ee482f2ab75c284d763d18";
    var EVIDENCE_BYTES = 5100;
    var REVIEWED_TIP = "99e6ed195c95b5cdcd27a6690f2441df4e7ae6a9";
    var VERDICT = "DRY_RUN_READY_FOR_SEPARATE_APPLY_AUTHORIZATION";
    var MARKER = "attempt-99e6ed195c95-aaa974cab7984e13a66e4e1237e211b9.marker";
    var PRECONDITION_SHA256 = "d2e47fb6c77501fa6a8b7e29ea728550c23f0daef1713ded7de96c080bcf8288";
    var PRECONDITION_OID = "f05bfa25def88f28037defbbdd3375bd84068604";
    var BUNDLE_OID = "2e42488b4888f8fe1a1cd25fdbcaa5d72594d84e";
    var BUNDLE_SHA256 = "6061df5e2689d8658169485beb0f66f3b73591de0161b4f47f65beb9eb8d4614";
    var BUNDLE_BYTES = 277613;
    var MIGRATIONS = [
      {
        version: "20260917044537",
        oid: "788de3e4b600c0aac57738c9ca3509d1b7e76d49",
    ...
    function assertPriorDryRunEvidencePublished({ auth, cwd, env, priorDryRunEvidencePath } = {}) {
      ...
      if (loaded.buffer[loaded.buffer.length - 1] !== 10 || loaded.buffer.length > 1 && loaded.buffer[loaded.buffer.length - 2] === 10) {
...
// scripts/security/ra-pro-accounting-automation-pre-apply-gates.js
var require_ra_pro_accounting_automation_pre_apply_gates = __commonJS({
  "scripts/security/ra-pro-accounting-automation-pre-apply-gates.js"(exports2, module2) {
    "use strict";
    ...
    var WEBHOOK_STATUSES = ["received", "processing", "retryable"];
    ...
    var DISPOSABLE_VALIDATOR = "PASS_DISPOSABLE_PRE_APPLY_LIVE_VALIDATION";
    function blocked(code, message) {
      const error = new Error(`${code}: ${message}`);
      error.code = code;
...
var require_ra_pro_accounting_automation_apply_authorization = __commonJS({
  "scripts/security/ra-pro-accounting-automation-apply-authorization.js"(exports2, module2) {
    ...
    var RETIREMENT_PROTOCOL = "RA_PRO_ACCOUNTING_AUTOMATION_APPLY_ATTEMPT_RETIREMENT_V1";
    ...
    var RETIREMENT_KEYS = Object.freeze([
      "protocol",
      "status",
      "attempt_id",
    ...
    function listRetiredAttempts(auth) {
      const rows = auth && Array.isArray(auth.production_apply_attempt_retirements) ? auth.production_apply_attempt_retirements : [];
      return rows;
    }
    ...
    function buildAttemptRetirement(inputs = {}) {
      ...
      const retirement = {
        ...
        note: String(
          inputs.note || "Consumed without marker. Permanently non-reusable; absence of a marker does not restore the attempt."
        )
      };
    ...
    function revokeProductionApplyAuthorization(auth, retirementInputs = {}) {
      ...
      const retirement = buildAttemptRetirement({
        ...retirementInputs,
        attempt_id: attemptId,
        authorized_executable_commit: retirementInputs.authorized_executable_commit || record.authorized_executable_commit
      });
    ...
    function preflightApplyAuthorization2(inputs = {}) {
      try {
        const cwd = inputs.cwd || process.cwd();
        const publication = resolvePublicationCommit(inputs, cwd);
        const { auth } = loadAuthFromGit(publication, cwd);
        const { assertPriorDryRunEvidencePublished } = require_ra_pro_accounting_automation_prior_dry_run_gates();
        const { assertPreApplyLiveEvidencePublished } = require_ra_pro_accounting_automation_pre_apply_gates();
        ...
        if (inputs.authorizationToken != null && inputs.authorizationToken !== APPLY_AUTHORIZATION_TOKEN2) {
...
`;
    ...
    module2.exports = {
      ...
      listRetiredAttempts,
      preflightApplyAuthorization: preflightApplyAuthorization2,
      recheckApplyAuthorizationPin: recheckApplyAuthorizationPin2,
...
var require_ra_pro_accounting_automation_schema_probes = __commonJS({
  "scripts/security/ra-pro-accounting-automation-schema-probes.js"(exports2, module2) {
    ...
    var TARGET_TABLES = Object.freeze([
    ...
    var TARGET_INDEXES = Object.freeze([
  ...
  `);
      ...
      const targetTablesAbsent = checks.weekly_runs_absent && checks.weekly_findings_absent && checks.month_end_packages_absent;
      const targetFunctionsAbsent = checks.weekly_persist_absent && checks.month_end_persist_absent;
      const prerequisitesPresent = checks.firms_id_uuid_key && checks.companies_id_uuid_key && checks.firm_clients_id_uuid_key && checks.firm_clients_firm_id_uuid && checks.accounting_syncs_id_uuid_key && checks.firm_memberships_shape && checks.auth_uid_uuid && checks.roles_compatible && checks.gen_random_uuid_present;
    ...
    async function probeIdempotentPersistence(client) {
      ...
      try {
        ...
        const ok = weeklyFirst.rows[0].reused === false && weeklySecond.rows[0].reused === true && weeklyFirst.rows[0].run_id === weeklySecond.rows[0].run_id && monthFirst.rows[0].reused === false && monthSecond.rows[0].reused === true && monthFirst.rows[0].package_id === monthSecond.rows[0].package_id;
    ...
    `);
        ...
        view.functions_present = Boolean(
          weeklyFn && monthFn && functions.rows[0].weekly_sig === true && functions.rows[0].month_sig === true && weeklyFn.prosecdef === false && monthFn.prosecdef === false && String(weeklyFn.config || "").includes("search_path") && String(monthFn.config || "").includes("search_path")
        );
    ...
    `);
        ...
        view.grants_match = Boolean(
          grant.auth_select === true && grant.auth_insert === false && grant.auth_update === false && grant.auth_delete === false && grant.anon_select === false && grant.svc_select === true && grant.svc_insert === true && grant.svc_update === false && grant.svc_delete === false && grant.findings_auth_insert === false && grant.month_auth_select === true && grant.month_auth_insert === false && grant.month_svc_insert === true && grant.weekly_exec === true && grant.weekly_auth_exec === false && grant.weekly_anon_exec === false && grant.month_exec === true && grant.month_auth_exec === false
        );
    ...
    `);
        view.verification_rows_rolled_back = leftover.rows[0].runs === 0 && leftover.rows[0].packages === 0;
...
var require_ra_pro_accounting_automation_apply_core = __commonJS({
  "scripts/security/ra-pro-accounting-automation-apply-core.js"(exports2, module2) {
    ...
    var {
      assertPriorDryRunEvidencePublished
    } = require_ra_pro_accounting_automation_prior_dry_run_gates();
    var {
      assertPreApplyLiveEvidencePublished
    ...
    var {
      ...
      collectDryRunSchemaProbes,
      verifyPostCommit
    } = require_ra_pro_accounting_automation_schema_probes();
      ...
      try {
        ...
        if (inputs.allowDisposablePublicationCommit !== true) {
          ...
          if (pinShape) {
            ...
            const probe = preflightApplyAuthorization2({
              cwd: resolveRepoRoot(inputs),
              now: inputs.now
            });
            if (!probe.blocked) {
...
`);
  ...
  const result = await runApplicator({
    ...
    existingMarkerPath: flags.applyMarker || void 0,
    authorizationPin: flags.authorizationPin || void 0,
    env: process.env,
    argv: process.argv
  });
