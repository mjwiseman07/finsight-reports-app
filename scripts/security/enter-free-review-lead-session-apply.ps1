...
<#
.SYNOPSIS
  Native entry trust-root helper for Free Review lead-session apply (pre-Node).
...
.DESCRIPTION
  ...
  FREE_REVIEW_LEAD_SESSION_APPLY_DATABASE_URL values.
#>
[CmdletBinding()]
param(
...
function Write-EntryEvidence {
  param($Object)
  # Emit exactly one FRLS_LEAD_SESSION_EVIDENCE_V1 frame. Do not probe missing
...
function Get-TrustedWindowsPowerShell {
  ...
  if ($item.Attributes -band [System.IO.FileAttributes]::ReparsePoint) { throw "powershell.exe reparse point" }
...
try {
  ...
  $authFile = Join-Path $repo $authPath
...
$tempDir = Join-Path ([System.IO.Path]::GetTempPath()) ("frls-enter-" + [guid]::NewGuid().ToString("N"))
...
try {
  ...
  $psi = New-Object System.Diagnostics.ProcessStartInfo
  $psi.FileName = Get-TrustedWindowsPowerShell
  $psi.UseShellExecute = $false
  $psi.RedirectStandardOutput = $true
  $psi.RedirectStandardError = $true
  $psi.CreateNoWindow = $true
  ...
  $removeKeys = @(
    ...
    "CONTAINMENT_APPLY_DATABASE_URL", "NODE_TLS_REJECT_UNAUTHORIZED"
  )
  $stripped = $false
