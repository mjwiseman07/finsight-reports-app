...
try {
  ...
  if (-not $EvidenceOutDir) {
    $EvidenceOutDir = Join-Path $env:TEMP ("ra-acct-enter-boot-" + [guid]::NewGuid().ToString("N"))
  }
  New-Item -ItemType Directory -Force -Path $EvidenceOutDir | Out-Null
  $script:MaterialRoot = Join-Path $EvidenceOutDir ("material-enter-boot-" + [guid]::NewGuid().ToString("N"))
  ...
  $vb = $auth.visible_ceremony_bootstrap
  if (-not $vb) { throw "missing visible_ceremony_bootstrap seals" }
  $bootDest = Join-Path $script:MaterialRoot "bootstrap-visible-ra-pro-accounting-automation-ceremony.ps1"
  [void](Assert-BlobSeal -Commit $bootSrc.ToLowerInvariant() -Rel $BootstrapRel -Seal $vb -Dest $bootDest -WorkDir $RepoRoot)
  ...
  if (-not [string]::IsNullOrWhiteSpace($TestSyntheticDatabaseUrl)) {
    $bootArgs += @("-TestSyntheticDatabaseUrl", $TestSyntheticDatabaseUrl)
  }
  if (-not [string]::IsNullOrWhiteSpace($TestHarnessChildStub)) {
    $bootArgs += @("-TestHarnessChildStub", $TestHarnessChildStub)
  }
  if ($ChildTimeoutMs -ne 120000) {
    $bootArgs += @("-ChildTimeoutMs", "$ChildTimeoutMs")
  }
  if (-not [string]::IsNullOrWhiteSpace($TestForcePrePromptNullIndex)) {
    $bootArgs += @("-TestForcePrePromptNullIndex", $TestForcePrePromptNullIndex)
  }
  if ($TestForceCleanupFailure) { $bootArgs += "-TestForceCleanupFailure" }
  if ($TestForceTerminateFailure) { $bootArgs += "-TestForceTerminateFailure" }

  $psi = New-Object Diagnostics.ProcessStartInfo
  $psi.FileName = $psExe
  $psi.Arguments = ($bootArgs | ForEach-Object { Format-Win32Argument $_ }) -join " "
  ...
  if (-not $p.WaitForExit([Math]::Max(60000, $ChildTimeoutMs + 60000))) {
    try { $p.Kill() } catch {}
    throw "sealed bootstrap child timed out"
  }
