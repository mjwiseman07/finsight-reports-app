...
<#
.SYNOPSIS
  Operator-assisted production dry-run ceremony for Free Review lead sessions (FRLS).
  Hidden SecureString URL injection + reviewed native entry.
  Child evidence must be FRLS_LEAD_SESSION_EVIDENCE_V1; never logs credentials.
#>
[CmdletBinding()]
param(
  ...
  # Test harness only: loopback synthetic URL when FRLS_CEREMONY_ALLOW_SYNTHETIC_URL=1
  [Parameter(Mandatory = $false)]
...
function Clear-FrlsCredential {
  Remove-Item Env:FREE_REVIEW_LEAD_SESSION_APPLY_DATABASE_URL -ErrorAction SilentlyContinue
  [Environment]::SetEnvironmentVariable("FREE_REVIEW_LEAD_SESSION_APPLY_DATABASE_URL", $null, "Process")
  # Retired channel — clear if a hostile process left it set.
  Remove-Item Env:FREE_REVIEW_LEAD_SESSION_APPLY_SSL_ROOTCERT -ErrorAction SilentlyContinue
  [Environment]::SetEnvironmentVariable("FREE_REVIEW_LEAD_SESSION_APPLY_SSL_ROOTCERT", $null, "Process")
...
function Sanitize-Text([string]$Text) {
  ...
  $t = [regex]::Replace($t, "FREE_REVIEW_LEAD_SESSION_APPLY_DATABASE_URL\s*[:=]\s*[^\s`"']+", "FREE_REVIEW_LEAD_SESSION_APPLY_DATABASE_URL=***")
...
function Get-HarnessContaminationEnvNames {
  # FRLS has no fixture Target#2 world; the synthetic-URL gate is the only harness channel.
  return @(
    "FRLS_CEREMONY_ALLOW_SYNTHETIC_URL"
  )
}
...
try {
  ...
  $authJson = Invoke-GitTextLocal @("cat-file", "blob", "${tip}:docs/security/free-review-lead-session-apply/TOOLING_AUTHORIZATION.json")
  ...
  $entryPath = Join-Path $entryTempDir "enter-free-review-lead-session-apply.ps1"
  ...
  $allowSyntheticGate = [Environment]::GetEnvironmentVariable("FRLS_CEREMONY_ALLOW_SYNTHETIC_URL", "Process") -eq "1"
  ...
  if ($hasExplicitSyntheticUrl) {
    if (-not $allowSyntheticGate) {
      throw "SYNTHETIC_URL_NOT_ALLOWED: set FRLS_CEREMONY_ALLOW_SYNTHETIC_URL=1 for harness only"
    }
    ...
    $secure = Read-Host -Prompt "FREE_REVIEW_LEAD_SESSION_APPLY_DATABASE_URL" -AsSecureString
  ...
  $env:FREE_REVIEW_LEAD_SESSION_APPLY_DATABASE_URL = $plain
  $plain = $null
  [GC]::Collect(); [GC]::WaitForPendingFinalizers()

  # Fail closed if retired CA-path channel is present (trust root is embedded).
  $hostileCa = [Environment]::GetEnvironmentVariable("FREE_REVIEW_LEAD_SESSION_APPLY_SSL_ROOTCERT", "Process")
  if (-not [string]::IsNullOrWhiteSpace($hostileCa)) {
    throw "BLOCKED_TLS_CA_PATH_FORBIDDEN: FREE_REVIEW_LEAD_SESSION_APPLY_SSL_ROOTCERT is retired; trust root is embedded"
  }
  ...
  # Separate stdout/stderr — never merge with 2>&1 (pollutes FRLS_LEAD_SESSION_EVIDENCE_V1 frame)
  $entryArgs = @(
  ...
  Write-Host "[3/3] Extracting FRLS_LEAD_SESSION_EVIDENCE_V1 frame..."
  $proto = Join-Path $EvidenceOutDir "free-review-lead-session-evidence.js"
  $decodeHelper = Join-Path $EvidenceOutDir "free-review-lead-session-evidence-decode-frame.js"
  Materialize-GitBlob "scripts/security/free-review-lead-session-evidence.js" $proto | Out-Null
  Materialize-GitBlob "scripts/security/free-review-lead-session-evidence-decode-frame.js" $decodeHelper | Out-Null
  ...
  $evidence = [ordered]@{
    ...
    credential_redaction_confirmation = [ordered]@{
      ...
      ca_path_env_absent = (-not [bool]$env:FREE_REVIEW_LEAD_SESSION_APPLY_SSL_ROOTCERT)
    ...
    cleanup = [ordered]@{
      credential_cleared = (-not [bool]$env:FREE_REVIEW_LEAD_SESSION_APPLY_DATABASE_URL)
      ca_path_env_absent = (-not [bool]$env:FREE_REVIEW_LEAD_SESSION_APPLY_SSL_ROOTCERT)
      retired_containment_channel_absent = (-not [bool]$env:CONTAINMENT_APPLY_DATABASE_URL)
  ...
  foreach ($f in @("free-review-lead-session-evidence-decode-frame.js", "free-review-lead-session-evidence.js", "free-review-lead-session-evidence-frame-tool.js", "containment-evidence-protocol.js")) {
