...
try {
  ...
  if ($PromptInputTimeoutMs -ne 600000) { $supArgs += @("-PromptInputTimeoutMs", "$PromptInputTimeoutMs") }
  if ($TestTimeoutBudgetProbe) { $supArgs += "-TestTimeoutBudgetProbe" }
  if ($TestForcePromptTimeout) { $supArgs += "-TestForcePromptTimeout" }
  ...
  $waitMs = Get-SupervisedWaitMs $PromptInputTimeoutMs $ChildTimeoutMs 32000 120000
  if (-not $p.WaitForExit($waitMs)) {
    ...
    Write-ParentFallbackEvidence "sealed supervisor child timed out" "forced_parent_termination"
    $evidenceFile = Join-Path $EvidenceOutDir "PRODUCTION_DRY_RUN_EVIDENCE.json"
    if (Test-Path -LiteralPath $evidenceFile) {
      Write-Output ([IO.File]::ReadAllText($evidenceFile).TrimEnd())
    } else {
      Write-Blocked "sealed supervisor child timed out"
    }
    exit 1
  }
  try { $stdout = [string]$outTask.Result } catch { $stdout = "" }
