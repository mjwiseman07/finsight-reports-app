...
$Freeze = $PrHead
$EntryRel = "scripts\security\enter-containment-apply.ps1"
$secure = $null
$bstr = [IntPtr]::Zero
$plain = $null
...
try {
  ...
  Write-Host "[2/3] Invoking native entry (dry-run)..."
  ...
  $bytesProto = & git -C $RepoRoot cat-file blob "${Freeze}:scripts/security/containment-evidence-protocol.js"
  $bytesTool = & git -C $RepoRoot cat-file blob "${Freeze}:scripts/security/containment-evidence-frame-tool.js"
  # Use Process for binary-safe materialization
  function Materialize-Blob([string]$Rel, [string]$Dest) {
    ...
    $p.WaitForExit()
    if ($p.ExitCode -ne 0) { throw "failed to materialize $Rel" }
    [IO.File]::WriteAllBytes($Dest, $ms.ToArray())
  }
  Materialize-Blob "scripts/security/containment-evidence-protocol.js" $proto
  Materialize-Blob "scripts/security/containment-evidence-frame-tool.js" $tool
