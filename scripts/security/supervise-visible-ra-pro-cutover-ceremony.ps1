...
.DESCRIPTION
  ...
  EvidenceOutDir/material-supervisor-*/enter-visible-ra-pro-cutover-ceremony.ps1,
  ...
  Residual: if THIS supervisor is hard-killed, no RA_PRO_CUTOVER_EVIDENCE_V1 can
  ...
  Never sets RA_PRO_CUTOVER_APPLY_DATABASE_URL. Synthetic -TestStubScript only.
#>
[CmdletBinding()]
param(
...
# Fixed type name so callers always use [RaProCutoverVisible.JobApi]::Method
if (-not ("RaProCutoverVisible.JobApi" -as [type])) {
  Add-Type -TypeDefinition @"
using System;
using System.Text;
using System.Runtime.InteropServices;

namespace RaProCutoverVisible {
...
function New-KillOnCloseJob {
  $h = [RaProCutoverVisible.JobApi]::CreateJobObject([IntPtr]::Zero, $null)
  if ($h -eq [IntPtr]::Zero) {
    throw "CreateJobObject failed err=$([Runtime.InteropServices.Marshal]::GetLastWin32Error())"
  }
  $info = New-Object "RaProCutoverVisible.JobApi+JOBOBJECT_EXTENDED_LIMIT_INFORMATION"
  $basic = $info.BasicLimitInformation
  # ONLY kill-on-close — never BREAKAWAY / silent breakaway flags.
  $basic.LimitFlags = [RaProCutoverVisible.JobApi]::JOB_OBJECT_LIMIT_KILL_ON_JOB_CLOSE
  ...
  try {
    ...
    $ok = [RaProCutoverVisible.JobApi]::SetInformationJobObject(
      $h,
      [RaProCutoverVisible.JobApi]::JobObjectExtendedLimitInformation,
      $ptr,
      [uint32]$size
    )
    if (-not $ok) {
    ...
    try { [void][RaProCutoverVisible.JobApi]::CloseHandle($h) } catch {}
    throw
  } finally {
...
function Start-SuspendedInJob {
  ...
  $si = New-Object "RaProCutoverVisible.JobApi+STARTUPINFO"
  $si.cb = [uint32][Runtime.InteropServices.Marshal]::SizeOf($si)
  $pi = New-Object "RaProCutoverVisible.JobApi+PROCESS_INFORMATION"
  $cmd = New-Object System.Text.StringBuilder
  [void]$cmd.Append($CommandLine)
  $flags = [RaProCutoverVisible.JobApi]::CREATE_SUSPENDED
  $ok = [RaProCutoverVisible.JobApi]::CreateProcess(
    $Exe,
    $cmd,
    [IntPtr]::Zero,
    [IntPtr]::Zero,
  ...
  try {
    ...
    $assigned = [RaProCutoverVisible.JobApi]::AssignProcessToJobObject($Job, $pi.hProcess)
    if (-not $assigned) {
      throw "AssignProcessToJobObject failed err=$([Runtime.InteropServices.Marshal]::GetLastWin32Error())"
    }
    $resume = [RaProCutoverVisible.JobApi]::ResumeThread($pi.hThread)
    ...
    try { [void][RaProCutoverVisible.JobApi]::TerminateJobObject($Job, 1) } catch {}
    try { [void][RaProCutoverVisible.JobApi]::CloseHandle($pi.hThread) } catch {}
    try { [void][RaProCutoverVisible.JobApi]::CloseHandle($pi.hProcess) } catch {}
    throw
  }
  try { [void][RaProCutoverVisible.JobApi]::CloseHandle($pi.hThread) } catch {}
...
function Close-JobAndConfirm {
  ...
  if ($script:JobHandle -ne [IntPtr]::Zero) {
    try {
      [void][RaProCutoverVisible.JobApi]::TerminateJobObject($script:JobHandle, 1)
    } catch {}
    try {
      [void][RaProCutoverVisible.JobApi]::CloseHandle($script:JobHandle)
      $jobClosed = $true
    } catch {}
    $script:JobHandle = [IntPtr]::Zero
  }
...
try {
  ...
  $entryDest = Join-Path $script:SupervisorMaterialRoot "enter-visible-ra-pro-cutover-ceremony.ps1"

  # Trust root: materialize entry from TIP blob only (never execute worktree entry as authority).
  try {
    [void](Assert-BlobSeal -Commit $tip -Rel "scripts/security/enter-visible-ra-pro-cutover-ceremony.ps1" -Seal $ve -Dest $entryDest -WorkDir $RepoRoot)
  } catch {
