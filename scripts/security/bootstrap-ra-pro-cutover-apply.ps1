...
<#
.SYNOPSIS
  Pre-Node sealed bootstrap for RA Pro billing-company cutover apply.

.DESCRIPTION
  Must be materialized from a committed Git blob and hash-verified before execution.
  Does not connect to PostgreSQL, read RA_PRO_CUTOVER_APPLY_DATABASE_URL values into
...
function Write-BootstrapEvidence {
  param($Object)
  # Emit exactly one RA_PRO_CUTOVER_EVIDENCE_V1 frame on stdout.
  $json = $Object | ConvertTo-Json -Depth 20 -Compress
  $bytes = [System.Text.Encoding]::UTF8.GetBytes($json)
  $frame = "RA_PRO_CUTOVER_EVIDENCE_V1:" + (ConvertTo-Base64Url -Bytes $bytes)
...
try {
  ...
  if ([string]$bundle.path -ne "scripts/security/bundles/ra-pro-cutover-applicator.standalone.cjs") {
  ...
  $dbEnvName = "RA_PRO_CUTOVER_APPLY_DATABASE_URL"
  ...
  $caEnvName = "RA_PRO_CUTOVER_APPLY_SSL_ROOTCERT"
  ...
  if ($caPresent) {
    $script:cleanupResult = Clear-TempPath -Path $script:tempRoot
    Stop-Bootstrap -Code "BLOCKED_TLS_CA_PATH_FORBIDDEN" -Phase "tls_policy" -Message "RA_PRO_CUTOVER_APPLY_SSL_ROOTCERT is retired; trust root is embedded" -Extra @{ tipHead = $tipHead; freeze = $freeze; cleanup = $script:cleanupResult }
  }
  ...
  $childEnv["RA_PRO_CUTOVER_ATTESTED_FREEZE"] = $freeze
  $childEnv["RA_PRO_CUTOVER_GIT_CWD"] = $RepoRoot
  ...
  if ($containmentDbPresent) {
    $script:cleanupResult = Clear-TempPath -Path $script:tempRoot
    Stop-Bootstrap -Code "PROHIBITED_CREDENTIAL_CHANNEL" -Phase "uri_validate" -Message "CONTAINMENT_APPLY_DATABASE_URL is forbidden for RA Pro cutover applicator" -Extra @{ tipHead = $tipHead; freeze = $freeze; cleanup = $script:cleanupResult }
  }
  $prohibitedNodeEnv = @(
  ...
  if ($stderr) {
    ...
    $scrubbedStderr = [regex]::Replace($scrubbedStderr, "RA_PRO_CUTOVER_APPLY_DATABASE_URL\s*[:=]\s*\S+", "RA_PRO_CUTOVER_APPLY_DATABASE_URL=***")
  }
  ...
  $protoFile = Join-Path $parseDir "ra-pro-cutover-evidence.js"
  $sharedProtoFile = Join-Path $parseDir "containment-evidence-protocol.js"
  $toolFile = Join-Path $parseDir "ra-pro-cutover-evidence-frame-tool.js"
  try {
    [System.IO.File]::WriteAllText($stdoutFile, [string]$stdout)
    $protoBytes = Invoke-GitBytes -GitArgs @("cat-file", "blob", "${freeze}:scripts/security/ra-pro-cutover-evidence.js") -WorkDir $RepoRoot
    # ra-pro-cutover-evidence.js requires ./containment-evidence-protocol — must be a sibling in parseDir
    ...
    $toolBytes = Invoke-GitBytes -GitArgs @("cat-file", "blob", "${freeze}:scripts/security/ra-pro-cutover-evidence-frame-tool.js") -WorkDir $RepoRoot
    ...
    if (-not $frameOut -or -not $frameOut.Trim().StartsWith("RA_PRO_CUTOVER_EVIDENCE_V1:")) {
