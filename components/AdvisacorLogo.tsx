...
type AdvisacorLogoVariant = "dark" | "light";
...
export function AdvisacorLogo({
  ...
  className?: string;
  priority?: boolean;
  variant?: AdvisacorLogoVariant;
}) {
  const src = variant === "light" ? "/advisacor-logo-light.svg" : "/advisacor-logo.svg";
...
export function AdvisacorLogoMark({
  className = "",
  variant = "dark",
}: {
  className?: string;
  ...
  const navy = variant === "light" ? "#FFFFFF" : "#070D20";
  ...
  return (
    <div className={`flex h-10 w-10 items-center justify-center ${className}`}>
      <svg width="26" height="26" viewBox="0 0 104 89" fill="none" aria-hidden="true">
        <path d="M50 0L104 89H88L50 24L16 89H0L50 0Z" fill={navy} />
        <path d="M50 0L31 37L42 51L50 35L88 89H104L50 0Z" fill="#D28A4A" />
        <path d="M30 65H44V89H30V65Z" fill={navy} />
        <path d="M53 49H68V89H53V49Z" fill={navy} />
