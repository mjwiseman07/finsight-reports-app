...
// Package levels are now sourced dynamically from lib/product-tiers.js.
// Consumers previously importing `superAdminPackageLevels` should switch to
// `getSuperAdminPackageLevels()` (flat) or `getSuperAdminPackageGroups()` (grouped).
import {
  getSuperAdminPackageLevels as _getSuperAdminPackageLevels,
  getSuperAdminPackageGroups as _getSuperAdminPackageGroups,
} from "./product-tiers.js";

/**
 * Returns the flat list of valid package-level tier keys for super-admin actions.
 * @returns {string[]}
 */
export function getSuperAdminPackageLevels() {
  return _getSuperAdminPackageLevels();
}

/**
 * Returns grouped tier keys for the /admin tier switcher UI.
 * @returns {{ v14Live: string[], v14Testing: string[], v14ComingSoon: string[], legacyOwner: string[], archived: string[] }}
 */
export function getSuperAdminPackageGroups() {
  return _getSuperAdminPackageGroups();
}
...
export const superAdminCapabilities = [
  ...
  "View customer test account in read-only customer mode",
  "Access all feature workflows",
  "Generate test PDFs",
  "Generate test PowerPoints",
  "Validate normalized ERP/accounting integration contracts",
