...
import { build as buildAssertionCoverage } from "./sections/assertion_coverage";
...
export const SECTION_PIPELINE = [
  ...
  { key: "assertion_coverage", builder: buildAssertionCoverage, ai: false },
...
async function loadRenderContext(supabase, closePeriodId) {
  ...
  // MC-2b.1 (Issue #6): resolve the connected accounting_connections
  // home_currency for this firm_client's owner. Nullable → falls back to
  // DEFAULT_FALLBACK_CURRENCY at the section-render layer. Read-only,
  // additive — does not modify the token-resolver contract.
  ...
  if (firmClient?.owner_user_id) {
    const { data: connRow } = await supabase
      ...
      .limit(1)
      .maybeSingle();
    if (connRow?.home_currency) {
      homeCurrency = String(connRow.home_currency).toUpperCase();
    }
  }

  return { closePeriod, firmClient, firm, checklistRun, homeCurrency };
}
...
export async function renderPacket({
  ...
  const ctx = {
    ...context, // includes homeCurrency from loadRenderContext
    preparer,
    reviewer,
    version,
    supabase,
