...
/**
 * Run an expected SQL failure inside SAVEPOINT / ROLLBACK TO / RELEASE so the
 * outer suite transaction is not left in SQLSTATE 25P02.
 * After containment, probes transaction health with SELECT 1.
 *
 * @param {import('pg').Client} client
 * @param {string} savepointName  // must match ^[a-zA-Z_][a-zA-Z0-9_]*$
 * @param {() => Promise<unknown>} operation
 * @returns {Promise<{ contained: true, error: unknown, transactionHealthy: true }>}
 */
async function runExpectedSqlFailureInSavepoint(client, savepointName, operation) {
  const sp = String(savepointName || "");
  if (!/^[a-zA-Z_][a-zA-Z0-9_]*$/.test(sp)) {
    throw new Error(`invalid_savepoint_name:${sp}`);
  }
  ...
  await probeJeReuseTransactionHealth(client);
  if (!caught) {
    throw new Error(`expected_sql_failure_did_not_occur:${sp}`);
  }
  return { contained: true, error: caught, transactionHealthy: true };
}

/**
 * Fail closed if the shared suite transaction is aborted (SQLSTATE 25P02 class).
 * @param {import('pg').Client} client
 */
async function probeJeReuseTransactionHealth(client) {
  const health = await client.query("SELECT 1::int AS ok");
  if (!health.rows[0] || health.rows[0].ok !== 1) {
    throw new Error("transaction_health_probe_failed");
  }
  return true;
}
