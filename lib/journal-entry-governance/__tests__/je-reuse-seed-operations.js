...
`.trim(),
  ...
  /**
   * Ledger publish scope parents (distinct from audit_ready_engagements).
   * ledger_events.engagement_id → public.engagements(id) (ledger_events_engagement_id_fkey).
   * Same synthetic UUID bridges JE custody (audit_ready) and ledger scope (engagements).
   * firm_clients.company_id is NOT NULL (D0) and must equal the seeded companies.id.
   */
  Object.freeze({
    name: "seed_firms",
    sql: `
INSERT INTO public.firms (id, name, is_demo)
VALUES ($1, 'JE3A Exec Reservation Test Firm', true)
ON CONFLICT (id) DO NOTHING
`.trim(),
    params: (ctx) => [ctx.ids.firm],
  }),
  Object.freeze({
    name: "seed_firm_clients",
    sql: `
INSERT INTO public.firm_clients (id, firm_id, company_id, name, is_demo)
VALUES ($1, $2, $3, 'JE3A Exec Reservation Test Client', true)
ON CONFLICT (id) DO NOTHING
`.trim(),
    params: (ctx) => [ctx.ids.firmClient, ctx.ids.firm, ctx.ids.company],
  }),
  Object.freeze({
    name: "seed_engagements",
    sql: `
INSERT INTO public.engagements (
  id, firm_id, engagement_name, engagement_type, status, scope_json
) VALUES (
  $1, $2, 'JE3A Exec Reservation Engagement', 'audit_prep', 'active', '{}'::jsonb
) ON CONFLICT (id) DO NOTHING
`.trim(),
    params: (ctx) => [ctx.ids.engagement, ctx.ids.firm],
  }),
  Object.freeze({
