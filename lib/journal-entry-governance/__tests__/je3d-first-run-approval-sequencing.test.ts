...
describe("JE-3D staged vs approved sequencing", () => {
  ...
  it("2. staged account IDs exist while account approval=false", () => {
  ...
  it("3. staging validation passes while both approval flags=false", () => {
  ...
  it("7. kill switch OFF + account approval=false blocks dispatch", () => {
    expect(
      isFirstRunDispatchAuthorized({
        ...
        accountEvidence: resolveFirstRunExplicitAccountEvidence(),
        killSwitchActive: false,
      }),
    ).toBe(false);
    expect(
      validateExplicitFirstRunAccounts({
        evidence: resolveFirstRunExplicitAccountEvidence(),
        mirrorRows,
      }).ok,
    ).toBe(false);
  });
