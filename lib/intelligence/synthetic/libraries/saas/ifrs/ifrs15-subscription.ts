...
export function resolveIfrs15SaasHandles(ctx: { containsSaaSARRData?: boolean }) {
  assertContainsSaaSARRData(ctx);
 return IFRS15_SAAS_HANDLES.map((h) => resolveSaasCitationHandle(h)); }
export function evaluateIfrs15Subscription(ctx: { containsSaaSARRData?: boolean }, input: { highlyProbable: boolean; usProbableOnly: boolean }) {
  assertContainsSaaSARRData(ctx);
