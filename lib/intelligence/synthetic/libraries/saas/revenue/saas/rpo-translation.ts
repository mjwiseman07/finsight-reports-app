...
export const MODULE_HANDLES = ["SaaS.Metrics.RPO", "ASC.606-10-50-13"] as const;
...
export function translateRpoToArr(ctx: { containsSaaSARRData?: boolean }, input: { rpo: number; avgContractMonths: number }) {
  assertContainsSaaSARRData(ctx);
  return { impliedArr: (input.rpo / Math.max(input.avgContractMonths, 1)) * 12 };
}
