...
export type SyntheticSageIntacctMappingValidationStatus = "not_run" | "passed" | "failed";

export interface BuildSageIntacctAdapterInput {
  ...
  multiEntityStructureReferenceId?: string;
  intacctDimensionMappingReferenceId?: string;
  locationDimensionMappingReferenceId?: string;
  departmentDimensionMappingReferenceId?: string;
  projectDimensionMappingReferenceId?: string;
  customDimensionMappingReferenceIds?: string[];
  ...
  canReadMultiEntityStructure?: boolean;
  canValidateMappings?: boolean;
  canSubmitJournalEntry?: boolean;
  canFetchPostingStatus?: boolean;
  mappingValidationStatus?: SyntheticSageIntacctMappingValidationStatus;
...
export interface SyntheticSageIntacctAdapter {
  sageIntacctAdapterId: string;
  sageIntacctAdapterKey: string;
...
export interface BuildSageIntacctAdapterResult {
  sageIntacctAdapter: SyntheticSageIntacctAdapter | null;
  skipped: boolean;
  warnings: string[];
}
...
function getBoundPhase38SnapshotHash(input: BuildSageIntacctAdapterInput): string {
  return input.boundPhase38SnapshotHash ?? input.phase38Handoff?.phase38SnapshotHash ?? "";
}

function getBoundPhase37SnapshotHash(input: BuildSageIntacctAdapterInput): string {
  return input.boundPhase37SnapshotHash ?? input.phase38Handoff?.boundPhase37SnapshotHash ?? "";
}

function getCompanyId(input: BuildSageIntacctAdapterInput): string {
  return input.companyId ?? input.phase38Handoff?.companyId ?? "";
}

function getScope(input: BuildSageIntacctAdapterInput): SyntheticAuditScope | undefined {
  return input.scope ?? input.phase38Handoff?.scope;
}

function getCustomerIsolation(input: BuildSageIntacctAdapterInput): SyntheticMemoryObjectIsolationDimension | undefined {
  return input.customerIsolation ?? input.phase38Handoff?.customerIsolation;
}

function getFirmIsolation(input: BuildSageIntacctAdapterInput): SyntheticMemoryObjectIsolationDimension | undefined {
  return input.firmIsolation ?? input.phase38Handoff?.firmIsolation;
}

function getClientIsolation(input: BuildSageIntacctAdapterInput): SyntheticMemoryObjectIsolationDimension | undefined {
  return input.clientIsolation ?? input.phase38Handoff?.clientIsolation;
}

function getPhase39StaleMarker(input: BuildSageIntacctAdapterInput): SyntheticPhase38StaleMarker {
  return input.phase39StaleMarker ?? input.phase38Handoff?.phase38StaleMarker ?? "current";
}

function getDerivationMethod(input: BuildSageIntacctAdapterInput): SyntheticActionDerivationMethod {
  return input.derivationMethod ?? input.phase38Handoff?.derivationMethod ?? "handoff_metadata_preservation";
}

function getMappingValidationStatus(input: BuildSageIntacctAdapterInput): SyntheticSageIntacctMappingValidationStatus {
  return input.mappingValidationStatus ?? "not_run";
}

function getAdapterConfigurationComplete(input: BuildSageIntacctAdapterInput): boolean {
  ...
  return (
    ...
    hasValue(input.multiEntityStructureReferenceId) &&
    hasValue(input.intacctDimensionMappingReferenceId)
  );
}

function collectMissingRequiredIdentifiers(input: BuildSageIntacctAdapterInput): string[] {
...
function buildDerivationHash(input: BuildSageIntacctAdapterInput): string {
  return stableSnapshotHash({
    ...
    contraAccountSignAwareHandlingRequired: true,
    respectsSourceSystemClassification: true,
    multiEntityStructureReferenceId: input.multiEntityStructureReferenceId ?? "",
    intacctDimensionMappingReferenceId: input.intacctDimensionMappingReferenceId ?? "",
    locationDimensionMappingReferenceId: input.locationDimensionMappingReferenceId ?? "",
    departmentDimensionMappingReferenceId: input.departmentDimensionMappingReferenceId ?? "",
    projectDimensionMappingReferenceId: input.projectDimensionMappingReferenceId ?? "",
    customDimensionMappingReferenceIds: getInputArray(input.customDimensionMappingReferenceIds),
    ...
    canReadMultiEntityStructure: input.canReadMultiEntityStructure === true,
...
export function buildSageIntacctAdapter(input: BuildSageIntacctAdapterInput): BuildSageIntacctAdapterResult {
  const warnings = [...getInputArray(input.warnings)];
  const missingRequiredIdentifiers = collectMissingRequiredIdentifiers(input);

  if (missingRequiredIdentifiers.length > 0) {
    return {
      sageIntacctAdapter: null,
      skipped: true,
      warnings: [
        ...warnings,
  ...
  const sageIntacctAdapterKey = stableSnapshotHash({
    erpType: "sage_intacct",
    adapterKind: "native",
    companyId,
  ...
  const sageIntacctAdapterId = stableSnapshotHash({
    sageIntacctAdapterKey,
    artifactType: "SyntheticSageIntacctAdapter",
  });

  return {
    sageIntacctAdapter: {
      sageIntacctAdapterId,
      sageIntacctAdapterKey,
      erpType: "sage_intacct",
      adapterKind: "native",
      erpAdapterContractReferenceId,
