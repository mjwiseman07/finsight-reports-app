...
export type SyntheticSeniorAccountantAssignedCapability =
  | "close_ownership"
  | "revenue_review"
  | "margin_analysis"
  | "flux_analysis"
  ...
  | "staff_review"
  | "controller_ready_summary";

export interface BuildSeniorAccountantRoleInput {
  ...
  assignedCapabilities?: SyntheticSeniorAccountantAssignedCapability[];
...
export interface SyntheticSeniorAccountantRole {
  seniorAccountantRoleId: string;
  seniorAccountantRoleKey: string;
  ...
  canReviewStaffAccountantOutput: true;
...
export interface BuildSeniorAccountantRoleResult {
  seniorAccountantRole: SyntheticSeniorAccountantRole | null;
  skipped: boolean;
  warnings: string[];
}

export const SENIOR_ACCOUNTANT_ASSIGNED_CAPABILITIES: SyntheticSeniorAccountantAssignedCapability[] = [
  "close_ownership",
  "revenue_review",
  "margin_analysis",
  "flux_analysis",
...
function getBoundPhase38SnapshotHash(input: BuildSeniorAccountantRoleInput): string {
  return input.boundPhase38SnapshotHash ?? input.phase38Handoff?.phase38SnapshotHash ?? "";
}

function getBoundPhase37SnapshotHash(input: BuildSeniorAccountantRoleInput): string {
  return input.boundPhase37SnapshotHash ?? input.phase38Handoff?.boundPhase37SnapshotHash ?? "";
}

function getCompanyId(input: BuildSeniorAccountantRoleInput): string {
  return input.companyId ?? input.phase38Handoff?.companyId ?? "";
}

function getScope(input: BuildSeniorAccountantRoleInput): SyntheticAuditScope | undefined {
  return input.scope ?? input.phase38Handoff?.scope;
}

function getCustomerIsolation(input: BuildSeniorAccountantRoleInput): SyntheticMemoryObjectIsolationDimension | undefined {
  return input.customerIsolation ?? input.phase38Handoff?.customerIsolation;
}

function getFirmIsolation(input: BuildSeniorAccountantRoleInput): SyntheticMemoryObjectIsolationDimension | undefined {
  return input.firmIsolation ?? input.phase38Handoff?.firmIsolation;
}

function getClientIsolation(input: BuildSeniorAccountantRoleInput): SyntheticMemoryObjectIsolationDimension | undefined {
  return input.clientIsolation ?? input.phase38Handoff?.clientIsolation;
}

function getPhase39StaleMarker(input: BuildSeniorAccountantRoleInput): SyntheticPhase38StaleMarker {
  return input.phase39StaleMarker ?? input.phase38Handoff?.phase38StaleMarker ?? "current";
}

function getDerivationMethod(input: BuildSeniorAccountantRoleInput): SyntheticActionDerivationMethod {
  return input.derivationMethod ?? input.phase38Handoff?.derivationMethod ?? "handoff_metadata_preservation";
}

function getAssignedCapabilities(input: BuildSeniorAccountantRoleInput): SyntheticSeniorAccountantAssignedCapability[] {
  return input.assignedCapabilities ?? SENIOR_ACCOUNTANT_ASSIGNED_CAPABILITIES;
}

function getRoleCompositionComplete(input: BuildSeniorAccountantRoleInput): boolean {
...
function collectMissingRequiredIdentifiers(input: BuildSeniorAccountantRoleInput): string[] {
...
function buildDerivationHash(input: BuildSeniorAccountantRoleInput): string {
...
export function buildSeniorAccountantRole(input: BuildSeniorAccountantRoleInput): BuildSeniorAccountantRoleResult {
  const warnings = [...getInputArray(input.warnings)];
  const missingRequiredIdentifiers = collectMissingRequiredIdentifiers(input);

  if (missingRequiredIdentifiers.length > 0) {
    return {
      seniorAccountantRole: null,
      skipped: true,
      warnings: [
        ...warnings,
  ...
  const seniorAccountantRoleKey = stableSnapshotHash({
    roleType: "senior_accountant",
    roleInstanceId,
    companyId,
  ...
  const seniorAccountantRoleId = stableSnapshotHash({
    seniorAccountantRoleKey,
    artifactType: "SyntheticSeniorAccountantRole",
  });

  return {
    seniorAccountantRole: {
      seniorAccountantRoleId,
      seniorAccountantRoleKey,
      roleType: "senior_accountant",
      roleInstanceId,
      roleTemplateReferenceId,
