...
export function evaluateCas402(input: CasEvaluationInput, emitter: GcAuditEmitter): CasEvaluationResult {
  const handle = resolveGovConCitationHandle("CAS_402_CONSISTENCY_ALLOCATING");
  if (!input.structuralPreconditionsMet) {
    emitEscalationAudit(emitter, {
      code: "GOVCON_CAS_PRECONDITION_FAIL",
      message: `CAS 402 structural preconditions failed`,
    ...
    emitter.emitDcaaRateAudit({
      ...
      evidence: { casId: "402", url: handle.url },
