...
export function evaluateCas420(input: CasEvaluationInput, emitter: GcAuditEmitter): CasEvaluationResult {
  const handle = resolveGovConCitationHandle("CAS_420_IRAD_BP");
  if (!input.structuralPreconditionsMet) {
    emitEscalationAudit(emitter, {
      code: "GOVCON_CAS_PRECONDITION_FAIL",
      message: `CAS 420 structural preconditions failed`,
    ...
    emitter.emitDcaaRateAudit({
      ...
      evidence: { casId: "420", url: handle.url },
