...
export function evaluateAllowability_31_205_7(
  input: AllowabilityInput,
  emitter: GcAuditEmitter,
): AllowabilityResult {
  const handle = resolveGovConCitationHandle("FAR_31_205_7_CONTINGENCIES");
  if (!input.structuralPreconditionsMet) {
    const escalation = emitEscalationAudit(emitter, {
      code: "GOVCON_FAR_PRECONDITION_FAIL",
      message: `FAR 31.205-7 structural preconditions failed`,
    ...
    emitter.emitDcaaRateAudit({
      ...
      evidence: { subsection: "7", url: handle.url },
  ...
  emitter.emitDcaaRateAudit({
    subSegmentId: input.subSegmentId,
    decisionType: "far-31-205-allowability",
    evidence: { subsection: "7", url: handle.url, costCategory: input.costCategory },
    outcome: "allowed",
    handleId: handle.handleId,
  });
