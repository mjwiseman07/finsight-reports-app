...
export function applyAccountingSaaSOverlay(
  ...
  if (!input.evidencePresent) {
    throw Object.assign(new Error("SAAS_VERTICAL_OVERLAY_FAIL"), {
      escalationAudits: [{ channel: "escalation-audit", code: "SAAS_VERTICAL_OVERLAY_FAIL", message: "accounting-saas.ts" }],
    });
  }
  emitter.emitArrMrr("vertical-saas-overlay-applied", { overlay: "accounting-saas.ts", ...input });
  return { applied: true };
}
