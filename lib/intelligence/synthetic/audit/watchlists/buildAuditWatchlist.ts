...
export interface BuildAuditWatchlistInput {
  auditSurface: SyntheticAuditSurface | null;
}

export interface SyntheticAuditWatchlist {
  auditWatchlistId: string;
  ...
  watchlistCompatibility: SyntheticAuditSurfaceCompatibility;
  auditSurface: SyntheticAuditSurface;
  warnings: string[];
}

export interface BuildAuditWatchlistResult {
  auditWatchlist: SyntheticAuditWatchlist | null;
  skipped: boolean;
  warnings: string[];
}
...
function hasWatchlistTarget(surfaceCompatibility: SyntheticAuditSurfaceCompatibility | undefined): boolean {
  return surfaceCompatibility?.surfaceTargets.includes("watchlist") === true;
}

function buildAuditWatchlistId(auditSurface: SyntheticAuditSurface): string {
  return `synthetic-audit-watchlist:${stableSnapshotHash({
    ...
    watchlistReferenceIds: auditSurface.surfaceCompatibility.watchlistReferenceIds,
...
function validateAuditSurface(auditSurface: SyntheticAuditSurface | null): string[] {
  ...
  if (!auditSurface.surfaceCompatibility) {
    ...
    if (!hasWatchlistTarget(auditSurface.surfaceCompatibility)) {
      warnings.push('surfaceCompatibility.surfaceTargets must include "watchlist".');
    }
  }
...
export function buildAuditWatchlist(input: BuildAuditWatchlistInput): BuildAuditWatchlistResult {
  const warnings = validateAuditSurface(input.auditSurface);
  if (
    warnings.length > 0 ||
    !input.auditSurface ||
    !input.auditSurface.surfaceCompatibility ||
    !hasWatchlistTarget(input.auditSurface.surfaceCompatibility)
  ) {
    return {
      auditWatchlist: null,
      skipped: true,
  ...
  const watchlistCompatibility = auditSurface.surfaceCompatibility;

  return {
    auditWatchlist: {
      auditWatchlistId: buildAuditWatchlistId(auditSurface),
      auditSurfaceId: auditSurface.auditSurfaceId,
      auditConfidenceId: auditSurface.auditConfidenceId,
      auditFindingArtifactId: auditSurface.auditFindingArtifactId,
      auditFindingId: auditSurface.auditFindingId,
