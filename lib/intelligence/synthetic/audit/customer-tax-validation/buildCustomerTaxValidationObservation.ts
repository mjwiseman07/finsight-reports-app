...
export type SyntheticCustomerTaxValidationCategory =
  | "customer_tax_validation_candidate"
  | "customer_tax_relationship_candidate"
  | "customer_tax_documentation_candidate"
  | "customer_tax_support_candidate"
  | "customer_tax_jurisdiction_candidate"
  | "customer_tax_exemption_candidate"
  | "customer_tax_certificate_candidate"
  | "customer_tax_registration_candidate"
  | "customer_tax_classification_candidate"
  | "customer_tax_reconciliation_candidate"
  | "customer_tax_invoice_candidate"
  | "customer_tax_validation_intelligence_candidate"
  | "customer_sales_tax_candidate"
  | "customer_use_tax_candidate"
  | "customer_vat_candidate"
  | "customer_tax_rate_candidate"
  | "customer_taxability_candidate"
  | "customer_tax_timing_candidate"
  | "customer_tax_period_candidate"
  | "customer_tax_cutoff_candidate"
  | "customer_tax_relationship_change_candidate"
  | "customer_tax_anomaly_candidate"
  | "customer_tax_readiness_candidate"
  | "customer_tax_missing_candidate"
  | "customer_tax_pattern_candidate"
  | "customer_tax_multi_jurisdiction_candidate"
  | "customer_tax_certificate_expiry_candidate"
  | "customer_tax_treatment_change_candidate";

export const SYNTHETIC_CUSTOMER_TAX_VALIDATION_CATEGORIES: SyntheticCustomerTaxValidationCategory[] = [
  "customer_tax_validation_candidate",
  "customer_tax_relationship_candidate",
  "customer_tax_documentation_candidate",
  "customer_tax_support_candidate",
...
export interface SyntheticCustomerTaxValidationIsolationDimension {
  required: boolean;
  referenceIds: string[];
}

export interface SyntheticCustomerTaxValidationForwardCompatibleObservation {
  salesTaxRemittanceObservationId?: string;
  taxToInvoiceObservationId?: string;
  customerMasterObservationId?: string;
  customerComplianceObservationId?: string;
  customerTaxPatternObservationId?: string;
  customerTaxCertificateExpiryObservationId?: string;
  customerTaxMultiJurisdictionObservationId?: string;
...
export interface BuildCustomerTaxValidationObservationInput {
  auditContract: SyntheticAuditContract | null;
  customerTaxValidationObservationKey: string;
  customerTaxValidationCategory: SyntheticCustomerTaxValidationCategory;
  ...
  salesTaxRemittanceObservations?: SyntheticCustomerTaxValidationForwardCompatibleObservation[];
  taxToInvoiceObservations?: SyntheticCustomerTaxValidationForwardCompatibleObservation[];
  customerMasterObservations?: SyntheticCustomerTaxValidationForwardCompatibleObservation[];
  customerComplianceObservations?: SyntheticCustomerTaxValidationForwardCompatibleObservation[];
  customerTaxPatternObservations?: SyntheticCustomerTaxValidationForwardCompatibleObservation[];
  customerTaxCertificateExpiryObservations?: SyntheticCustomerTaxValidationForwardCompatibleObservation[];
  customerTaxMultiJurisdictionObservations?: SyntheticCustomerTaxValidationForwardCompatibleObservation[];
  taxMethodologyObservations?: SyntheticCustomerTaxValidationForwardCompatibleObservation[];
...
export interface SyntheticCustomerTaxValidationObservation {
  ...
  customerIsolation: SyntheticCustomerTaxValidationIsolationDimension;
  firmIsolation: SyntheticCustomerTaxValidationIsolationDimension;
  clientIsolation: SyntheticCustomerTaxValidationIsolationDimension;
  ...
  customerComplianceObservationIds: string[];
  customerTaxPatternObservationIds: string[];
  customerTaxCertificateExpiryObservationIds: string[];
  customerTaxMultiJurisdictionObservationIds: string[];
  ...
  auditWatchlistIds: string[];
  auditBriefingIds: string[];
  customerTaxValidationReferenceIds: string[];
...
export interface BuildCustomerTaxValidationObservationResult {
  customerTaxValidationObservation: SyntheticCustomerTaxValidationObservation | null;
  skipped: boolean;
  warnings: string[];
}
...
function isSupportedCustomerTaxValidationCategory(category: SyntheticCustomerTaxValidationCategory): boolean {
  return SYNTHETIC_CUSTOMER_TAX_VALIDATION_CATEGORIES.includes(category);
}

function buildCustomerIsolation(scope: SyntheticAuditScope): SyntheticCustomerTaxValidationIsolationDimension {
  return { required: scope.customerIsolationRequired, referenceIds: scope.isolationBoundaryIds };
}

function buildFirmIsolation(scope: SyntheticAuditScope): SyntheticCustomerTaxValidationIsolationDimension {
  return {
...
function buildClientIsolation(scope: SyntheticAuditScope): SyntheticCustomerTaxValidationIsolationDimension {
  return {
...
function getDomainObservations(input: BuildCustomerTaxValidationObservationInput): object[] {
  return [
    ...
    ...getInputArray(input.customerComplianceObservations),
    ...getInputArray(input.customerTaxPatternObservations),
    ...getInputArray(input.customerTaxCertificateExpiryObservations),
    ...getInputArray(input.customerTaxMultiJurisdictionObservations),
    ...getInputArray(input.taxMethodologyObservations),
  ];
}

function getAuditCandidates(input: BuildCustomerTaxValidationObservationInput): SyntheticAuditCandidate[] {
  return getInputArray(input.auditCandidates);
}

function getAuditEvidencePackages(input: BuildCustomerTaxValidationObservationInput): SyntheticAuditEvidencePackage[] {
  return getInputArray(input.auditEvidencePackages);
}

function getAuditFindings(input: BuildCustomerTaxValidationObservationInput): SyntheticAuditFinding[] {
  return getInputArray(input.auditFindings);
}

function getAuditConfidencePackages(input: BuildCustomerTaxValidationObservationInput): SyntheticAuditConfidence[] {
  return getInputArray(input.auditConfidencePackages);
}

function getAuditSurfaces(input: BuildCustomerTaxValidationObservationInput): SyntheticAuditSurface[] {
  return getInputArray(input.auditSurfaces);
}

function getAuditWatchlists(input: BuildCustomerTaxValidationObservationInput): SyntheticAuditWatchlist[] {
  return getInputArray(input.auditWatchlists);
}

function getAuditBriefings(input: BuildCustomerTaxValidationObservationInput): SyntheticAuditBriefing[] {
  return getInputArray(input.auditBriefings);
}

function getAllAuditArtifacts(input: BuildCustomerTaxValidationObservationInput): AuditArtifact[] {
  return [
...
function getPackagedAuditArtifacts(
  input: BuildCustomerTaxValidationObservationInput,
): Array<
  | SyntheticAuditEvidencePackage
  | SyntheticAuditFinding
  | SyntheticAuditConfidence
...
function getEvidenceReferenceIds(input: BuildCustomerTaxValidationObservationInput): string[] {
  return uniqueStable([
...
function getSourceReferenceIds(input: BuildCustomerTaxValidationObservationInput): string[] {
  return uniqueStable([
...
function getLineageReferenceIds(input: BuildCustomerTaxValidationObservationInput): string[] {
  return uniqueStable([
...
function getAuditContractReferenceIdsFromInput(input: BuildCustomerTaxValidationObservationInput): string[] {
  return uniqueStable([
...
function getReferenceIds(input: BuildCustomerTaxValidationObservationInput, singularName: string, arrayName: string): string[] {
  return uniqueStable([
...
function getCustomerTaxValidationReferenceIds(input: BuildCustomerTaxValidationObservationInput): string[] {
  return uniqueStable([
...
function buildCustomerTaxValidationObservationId(input: BuildCustomerTaxValidationObservationInput): string {
  return `synthetic-customer-tax-validation-observation:${stableSnapshotHash({
    customerTaxValidationObservationKey: input.customerTaxValidationObservationKey,
    customerTaxValidationCategory: input.customerTaxValidationCategory,
    ...
    customerComplianceObservationIds: getObservationIds(
      getInputArray(input.customerComplianceObservations),
      "customerComplianceObservationId",
    ),
    customerTaxPatternObservationIds: getObservationIds(
      getInputArray(input.customerTaxPatternObservations),
      "customerTaxPatternObservationId",
    ),
    customerTaxCertificateExpiryObservationIds: getObservationIds(
      getInputArray(input.customerTaxCertificateExpiryObservations),
      "customerTaxCertificateExpiryObservationId",
    ),
    customerTaxMultiJurisdictionObservationIds: getObservationIds(
      getInputArray(input.customerTaxMultiJurisdictionObservations),
      "customerTaxMultiJurisdictionObservationId",
    ),
    ...
    customerTaxValidationReferenceIds: getCustomerTaxValidationReferenceIds(input),
...
function validateAuditArtifacts(input: BuildCustomerTaxValidationObservationInput, warnings: string[]): void {
...
function validateDomainObservationIds(input: BuildCustomerTaxValidationObservationInput, warnings: string[]): void {
  const companyId = input.auditContract?.scope.companyId;
  for (const [observationName, observations, idField] of [
    ...
    ["customerComplianceObservations", getInputArray(input.customerComplianceObservations), "customerComplianceObservationId"],
    ["customerTaxPatternObservations", getInputArray(input.customerTaxPatternObservations), "customerTaxPatternObservationId"],
    [
      "customerTaxCertificateExpiryObservations",
      getInputArray(input.customerTaxCertificateExpiryObservations),
      "customerTaxCertificateExpiryObservationId",
    ],
    [
      "customerTaxMultiJurisdictionObservations",
      getInputArray(input.customerTaxMultiJurisdictionObservations),
      "customerTaxMultiJurisdictionObservationId",
    ],
...
function getForwardCompatibilityWarnings(input: BuildCustomerTaxValidationObservationInput): string[] {
  return [
    ...
    ...(getInputArray(input.customerMasterObservations).length > 0
      ...
      : []),
    ...(getInputArray(input.customerComplianceObservations).length > 0
      ? ["customer compliance intelligence is a forward-compatible Phase 34 module."]
      : []),
    ...(getInputArray(input.customerTaxPatternObservations).length > 0
      ? ["customer tax pattern intelligence is a forward-compatible Phase 34 module."]
      : []),
    ...(getInputArray(input.customerTaxCertificateExpiryObservations).length > 0
      ? ["customer tax certificate expiry intelligence is a forward-compatible Phase 34 module."]
      : []),
    ...(getInputArray(input.customerTaxMultiJurisdictionObservations).length > 0
      ? ["customer tax multi-jurisdiction intelligence is a forward-compatible Phase 34 module."]
      : []),
...
function validateInput(input: BuildCustomerTaxValidationObservationInput): string[] {
  ...
  if (!hasValue(input.customerTaxValidationObservationKey)) {
    warnings.push("customerTaxValidationObservationKey is required.");
  }
  if (!hasValue(input.customerTaxValidationCategory)) warnings.push("customerTaxValidationCategory is required.");
  if (!isSupportedCustomerTaxValidationCategory(input.customerTaxValidationCategory)) {
    warnings.push("customerTaxValidationCategory must be a supported customer tax validation category.");
  }
...
export function buildCustomerTaxValidationObservation(
  input: BuildCustomerTaxValidationObservationInput,
): BuildCustomerTaxValidationObservationResult {
  const fatalWarnings = validateInput(input);
  if (fatalWarnings.length > 0 || !input.auditContract) {
    return {
      customerTaxValidationObservation: null,
      skipped: true,
  ...
  return {
    customerTaxValidationObservation: {
      customerTaxValidationObservationId: buildCustomerTaxValidationObservationId(input),
      customerTaxValidationObservationKey: input.customerTaxValidationObservationKey,
      customerTaxValidationCategory: input.customerTaxValidationCategory,
      ...
      customerComplianceObservations: getInputArray(input.customerComplianceObservations),
      customerTaxPatternObservations: getInputArray(input.customerTaxPatternObservations),
      customerTaxCertificateExpiryObservations: getInputArray(input.customerTaxCertificateExpiryObservations),
      customerTaxMultiJurisdictionObservations: getInputArray(input.customerTaxMultiJurisdictionObservations),
