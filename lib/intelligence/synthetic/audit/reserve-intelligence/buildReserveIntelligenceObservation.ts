...
export type SyntheticReserveIntelligenceCategory =
  | "allowance_for_doubtful_accounts"
  | "inventory_reserve"
  | "warranty_reserve"
  | "returns_reserve"
  | "obsolescence_reserve"
  | "reserve_balance_change"
  | "reserve_percentage_change"
  | "reserve_relationship_candidate"
  | "reserve_trend_candidate"
  | "reserve_adequacy_candidate"
  | "reserve_reconciliation_risk"
  | "reserve_financial_statement_relationship";

export const SYNTHETIC_RESERVE_INTELLIGENCE_CATEGORIES: SyntheticReserveIntelligenceCategory[] = [
  "allowance_for_doubtful_accounts",
  "inventory_reserve",
  "warranty_reserve",
  "returns_reserve",
...
export interface BuildReserveIntelligenceObservationInput {
  auditContract: SyntheticAuditContract | null;
  reserveIntelligenceObservationKey: string;
  reserveIntelligenceCategory: SyntheticReserveIntelligenceCategory;
...
export interface SyntheticReserveIntelligenceObservation {
  reserveIntelligenceObservationId: string;
  reserveIntelligenceObservationKey: string;
  reserveIntelligenceCategory: SyntheticReserveIntelligenceCategory;
  companyId: string;
...
export interface BuildReserveIntelligenceObservationResult {
  reserveIntelligenceObservation: SyntheticReserveIntelligenceObservation | null;
  skipped: boolean;
  warnings: string[];
}
...
function isSupportedReserveIntelligenceCategory(category: SyntheticReserveIntelligenceCategory): boolean {
  return SYNTHETIC_RESERVE_INTELLIGENCE_CATEGORIES.includes(category);
}
...
function getAuditCandidates(input: BuildReserveIntelligenceObservationInput): SyntheticAuditCandidate[] {
  return input.auditCandidates ?? [];
}

function getAuditEvidencePackages(input: BuildReserveIntelligenceObservationInput): SyntheticAuditEvidencePackage[] {
  return input.auditEvidencePackages ?? [];
}

function getAuditFindings(input: BuildReserveIntelligenceObservationInput): SyntheticAuditFinding[] {
  return input.auditFindings ?? [];
}

function getAuditConfidencePackages(input: BuildReserveIntelligenceObservationInput): SyntheticAuditConfidence[] {
  return input.auditConfidencePackages ?? [];
}

function getAuditSurfaces(input: BuildReserveIntelligenceObservationInput): SyntheticAuditSurface[] {
  return input.auditSurfaces ?? [];
}

function getAuditWatchlists(input: BuildReserveIntelligenceObservationInput): SyntheticAuditWatchlist[] {
  return input.auditWatchlists ?? [];
}

function getAuditBriefings(input: BuildReserveIntelligenceObservationInput): SyntheticAuditBriefing[] {
  return input.auditBriefings ?? [];
}

function getAllAuditArtifacts(input: BuildReserveIntelligenceObservationInput): AuditArtifact[] {
  return [
...
function getPackagedAuditArtifacts(
  input: BuildReserveIntelligenceObservationInput,
): Array<
  | SyntheticAuditEvidencePackage
  | SyntheticAuditFinding
  | SyntheticAuditConfidence
...
function getEvidenceReferenceIds(input: BuildReserveIntelligenceObservationInput): string[] {
  return uniqueStable([
...
function getSourceReferenceIds(input: BuildReserveIntelligenceObservationInput): string[] {
  return uniqueStable([
...
function getLineageReferenceIds(input: BuildReserveIntelligenceObservationInput): string[] {
  return uniqueStable([
...
function getReserveReferenceIds(input: BuildReserveIntelligenceObservationInput): string[] {
  return uniqueStable([
...
function buildReserveIntelligenceObservationId(input: BuildReserveIntelligenceObservationInput): string {
  return `synthetic-reserve-intelligence-observation:${stableSnapshotHash({
    reserveIntelligenceObservationKey: input.reserveIntelligenceObservationKey,
    reserveIntelligenceCategory: input.reserveIntelligenceCategory,
    ...
    reserveReferenceIds: getReserveReferenceIds(input),
...
function validateArtifacts(input: BuildReserveIntelligenceObservationInput, warnings: string[]): void {
...
function validateInput(input: BuildReserveIntelligenceObservationInput): string[] {
  ...
  if (!hasValue(input.reserveIntelligenceObservationKey)) warnings.push("reserveIntelligenceObservationKey is required.");
  if (!hasValue(input.reserveIntelligenceCategory)) warnings.push("reserveIntelligenceCategory is required.");
  if (!isSupportedReserveIntelligenceCategory(input.reserveIntelligenceCategory)) {
    warnings.push("reserveIntelligenceCategory must be a supported reserve intelligence category.");
  }
...
export function buildReserveIntelligenceObservation(
  input: BuildReserveIntelligenceObservationInput,
): BuildReserveIntelligenceObservationResult {
  const warnings = validateInput(input);
  if (warnings.length > 0 || !input.auditContract) {
    return {
      reserveIntelligenceObservation: null,
      skipped: true,
      warnings,
    };
  }
  ...
  return {
    reserveIntelligenceObservation: {
      reserveIntelligenceObservationId: buildReserveIntelligenceObservationId(input),
      reserveIntelligenceObservationKey: input.reserveIntelligenceObservationKey,
      reserveIntelligenceCategory: input.reserveIntelligenceCategory,
