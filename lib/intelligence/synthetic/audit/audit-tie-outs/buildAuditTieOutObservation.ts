...
export type SyntheticAuditTieOutCategory =
  | "schedule_to_gl_candidate"
  | "schedule_to_trial_balance_candidate"
  | "schedule_to_financial_statement_candidate"
  | "subledger_to_gl_candidate"
  | "reconciliation_to_gl_candidate"
  | "support_to_schedule_candidate"
  | "rollforward_to_balance_candidate"
  | "tie_out_relationship_candidate"
  | "tie_out_completeness_candidate"
  | "tie_out_documentation_candidate"
  | "tie_out_reconciliation_candidate"
  | "audit_tie_out_candidate";

export const SYNTHETIC_AUDIT_TIE_OUT_CATEGORIES: SyntheticAuditTieOutCategory[] = [
  "schedule_to_gl_candidate",
  "schedule_to_trial_balance_candidate",
  "schedule_to_financial_statement_candidate",
  "subledger_to_gl_candidate",
...
export interface BuildAuditTieOutObservationInput {
  auditContract: SyntheticAuditContract | null;
  auditTieOutObservationKey: string;
  auditTieOutCategory: SyntheticAuditTieOutCategory;
...
export interface SyntheticAuditTieOutObservation {
  auditTieOutObservationId: string;
  auditTieOutObservationKey: string;
  auditTieOutCategory: SyntheticAuditTieOutCategory;
  companyId: string;
  ...
  auditTieOutReferenceIds: string[];
...
export interface BuildAuditTieOutObservationResult {
  auditTieOutObservation: SyntheticAuditTieOutObservation | null;
  skipped: boolean;
  warnings: string[];
}
...
function isSupportedAuditTieOutCategory(category: SyntheticAuditTieOutCategory): boolean {
  return SYNTHETIC_AUDIT_TIE_OUT_CATEGORIES.includes(category);
}
...
function getAuditScheduleObservations(input: BuildAuditTieOutObservationInput): SyntheticAuditScheduleObservation[] {
  return input.auditScheduleObservations ?? [];
}

function getPbcRequestObservations(input: BuildAuditTieOutObservationInput): SyntheticPbcRequestObservation[] {
  return input.pbcRequestObservations ?? [];
}

function getEvidenceSufficiencyObservations(
  input: BuildAuditTieOutObservationInput,
): SyntheticEvidenceSufficiencyObservation[] {
  return input.evidenceSufficiencyObservations ?? [];
}

function getAuditCoverageObservations(input: BuildAuditTieOutObservationInput): SyntheticAuditCoverageObservation[] {
  return input.auditCoverageObservations ?? [];
}

function getContinuousAuditObservations(input: BuildAuditTieOutObservationInput): SyntheticContinuousAuditObservation[] {
...
function getDomainObservations(input: BuildAuditTieOutObservationInput): DomainObservation[] {
  return [
...
function getAuditCandidates(input: BuildAuditTieOutObservationInput): SyntheticAuditCandidate[] {
  return input.auditCandidates ?? [];
}

function getAuditEvidencePackages(input: BuildAuditTieOutObservationInput): SyntheticAuditEvidencePackage[] {
  return input.auditEvidencePackages ?? [];
}

function getAuditFindings(input: BuildAuditTieOutObservationInput): SyntheticAuditFinding[] {
  return input.auditFindings ?? [];
}

function getAuditConfidencePackages(input: BuildAuditTieOutObservationInput): SyntheticAuditConfidence[] {
  return input.auditConfidencePackages ?? [];
}

function getAuditSurfaces(input: BuildAuditTieOutObservationInput): SyntheticAuditSurface[] {
  return input.auditSurfaces ?? [];
}

function getAuditWatchlists(input: BuildAuditTieOutObservationInput): SyntheticAuditWatchlist[] {
  return input.auditWatchlists ?? [];
}

function getAuditBriefings(input: BuildAuditTieOutObservationInput): SyntheticAuditBriefing[] {
  return input.auditBriefings ?? [];
}

function getAllAuditArtifacts(input: BuildAuditTieOutObservationInput): AuditArtifact[] {
  return [
...
function getEvidenceReferenceIds(input: BuildAuditTieOutObservationInput): string[] {
  return uniqueStable([
...
function getSourceReferenceIds(input: BuildAuditTieOutObservationInput): string[] {
  return uniqueStable([
...
function getLineageReferenceIds(input: BuildAuditTieOutObservationInput): string[] {
  return uniqueStable([
...
function getAuditContractReferenceIdsFromInput(input: BuildAuditTieOutObservationInput): string[] {
  return uniqueStable([
...
function getAuditCandidateIds(input: BuildAuditTieOutObservationInput): string[] {
  return uniqueStable([
...
function getAuditEvidencePackageIds(input: BuildAuditTieOutObservationInput): string[] {
  return uniqueStable([
    ...getAllAuditArtifacts(input).flatMap((artifact) => [
...
function getAuditFindingArtifactIds(input: BuildAuditTieOutObservationInput): string[] {
  return uniqueStable([
    ...getAllAuditArtifacts(input).flatMap((artifact) => [
...
function getAuditFindingIds(input: BuildAuditTieOutObservationInput): string[] {
  return uniqueStable([
    ...getAllAuditArtifacts(input).flatMap((artifact) => [
...
function getAuditConfidenceIds(input: BuildAuditTieOutObservationInput): string[] {
  return uniqueStable([
    ...getAllAuditArtifacts(input).flatMap((artifact) => [
...
function getAuditSurfaceIds(input: BuildAuditTieOutObservationInput): string[] {
  return uniqueStable([
    ...getAllAuditArtifacts(input).flatMap((artifact) => [
...
function getAuditWatchlistIds(input: BuildAuditTieOutObservationInput): string[] {
  return uniqueStable([
    ...getAllAuditArtifacts(input).flatMap((artifact) => [
...
function getAuditBriefingIds(input: BuildAuditTieOutObservationInput): string[] {
  return uniqueStable([
    ...getAllAuditArtifacts(input).flatMap((artifact) => [
...
function getAuditTieOutReferenceIds(input: BuildAuditTieOutObservationInput): string[] {
  return uniqueStable([
...
function buildAuditTieOutObservationId(input: BuildAuditTieOutObservationInput): string {
  return `synthetic-audit-tie-out-observation:${stableSnapshotHash({
    auditTieOutObservationKey: input.auditTieOutObservationKey,
    auditTieOutCategory: input.auditTieOutCategory,
    ...
    auditTieOutReferenceIds: getAuditTieOutReferenceIds(input),
...
function validateAuditArtifacts(input: BuildAuditTieOutObservationInput, warnings: string[]): void {
...
function validateDomainObservationIds(input: BuildAuditTieOutObservationInput, warnings: string[]): void {
  const companyId = input.auditContract?.scope.companyId;
...
function validateInput(input: BuildAuditTieOutObservationInput): string[] {
  ...
  if (!hasValue(input.auditTieOutObservationKey)) warnings.push("auditTieOutObservationKey is required.");
  if (!hasValue(input.auditTieOutCategory)) warnings.push("auditTieOutCategory is required.");
  if (!isSupportedAuditTieOutCategory(input.auditTieOutCategory)) {
    warnings.push("auditTieOutCategory must be a supported audit tie-out category.");
  }
...
export function buildAuditTieOutObservation(
  input: BuildAuditTieOutObservationInput,
): BuildAuditTieOutObservationResult {
  const warnings = validateInput(input);
  if (warnings.length > 0 || !input.auditContract) {
    return {
      auditTieOutObservation: null,
      skipped: true,
      warnings,
    };
  }
  ...
  return {
    auditTieOutObservation: {
      auditTieOutObservationId: buildAuditTieOutObservationId(input),
      auditTieOutObservationKey: input.auditTieOutObservationKey,
      auditTieOutCategory: input.auditTieOutCategory,
