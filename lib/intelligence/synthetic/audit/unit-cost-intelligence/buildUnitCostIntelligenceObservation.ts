...
export type SyntheticUnitCostIntelligenceCategory =
  | "unit_cost_change"
  | "unit_cost_increase_candidate"
  | "unit_cost_decrease_candidate"
  | "inventory_cost_relationship"
  | "vendor_cost_relationship"
  | "cost_trend_candidate"
  | "cost_variance_candidate"
  | "inventory_valuation_relationship"
  | "margin_relationship_candidate"
  | "reserve_relationship_candidate"
  | "cost_reconciliation_risk"
  | "cost_financial_statement_relationship";

export const SYNTHETIC_UNIT_COST_INTELLIGENCE_CATEGORIES: SyntheticUnitCostIntelligenceCategory[] = [
  "unit_cost_change",
  "unit_cost_increase_candidate",
  "unit_cost_decrease_candidate",
  "inventory_cost_relationship",
...
export interface BuildUnitCostIntelligenceObservationInput {
  auditContract: SyntheticAuditContract | null;
  unitCostIntelligenceObservationKey: string;
  unitCostIntelligenceCategory: SyntheticUnitCostIntelligenceCategory;
...
export interface SyntheticUnitCostIntelligenceObservation {
  unitCostIntelligenceObservationId: string;
  unitCostIntelligenceObservationKey: string;
  unitCostIntelligenceCategory: SyntheticUnitCostIntelligenceCategory;
  companyId: string;
...
export interface BuildUnitCostIntelligenceObservationResult {
  unitCostIntelligenceObservation: SyntheticUnitCostIntelligenceObservation | null;
  skipped: boolean;
  warnings: string[];
}
...
function isSupportedUnitCostIntelligenceCategory(category: SyntheticUnitCostIntelligenceCategory): boolean {
  return SYNTHETIC_UNIT_COST_INTELLIGENCE_CATEGORIES.includes(category);
}
...
function getAuditCandidates(input: BuildUnitCostIntelligenceObservationInput): SyntheticAuditCandidate[] {
  return input.auditCandidates ?? [];
}

function getAuditEvidencePackages(input: BuildUnitCostIntelligenceObservationInput): SyntheticAuditEvidencePackage[] {
  return input.auditEvidencePackages ?? [];
}

function getAuditFindings(input: BuildUnitCostIntelligenceObservationInput): SyntheticAuditFinding[] {
  return input.auditFindings ?? [];
}

function getAuditConfidencePackages(input: BuildUnitCostIntelligenceObservationInput): SyntheticAuditConfidence[] {
  return input.auditConfidencePackages ?? [];
}

function getAuditSurfaces(input: BuildUnitCostIntelligenceObservationInput): SyntheticAuditSurface[] {
  return input.auditSurfaces ?? [];
}

function getAuditWatchlists(input: BuildUnitCostIntelligenceObservationInput): SyntheticAuditWatchlist[] {
  return input.auditWatchlists ?? [];
}

function getAuditBriefings(input: BuildUnitCostIntelligenceObservationInput): SyntheticAuditBriefing[] {
  return input.auditBriefings ?? [];
}

function getAllAuditArtifacts(input: BuildUnitCostIntelligenceObservationInput): AuditArtifact[] {
  return [
...
function getPackagedAuditArtifacts(
  input: BuildUnitCostIntelligenceObservationInput,
): Array<
  | SyntheticAuditEvidencePackage
  | SyntheticAuditFinding
  | SyntheticAuditConfidence
...
function getEvidenceReferenceIds(input: BuildUnitCostIntelligenceObservationInput): string[] {
  return uniqueStable([
...
function getSourceReferenceIds(input: BuildUnitCostIntelligenceObservationInput): string[] {
  return uniqueStable([
...
function getLineageReferenceIds(input: BuildUnitCostIntelligenceObservationInput): string[] {
  return uniqueStable([
...
function getUnitCostReferenceIds(input: BuildUnitCostIntelligenceObservationInput): string[] {
  return uniqueStable([
    ...(input.auditContract?.observationMetadata?.relatedAccountIds ?? []),
    ...(input.auditContract?.observationMetadata?.relatedScheduleIds ?? []),
    ...getAllAuditArtifacts(input).flatMap((artifact) => artifact.observationMetadata?.relatedAccountIds ?? []),
    ...getAllAuditArtifacts(input).flatMap((artifact) => artifact.observationMetadata?.relatedScheduleIds ?? []),
    ...getSourceReferenceIds(input),
  ]);
}

function buildUnitCostIntelligenceObservationId(input: BuildUnitCostIntelligenceObservationInput): string {
  return `synthetic-unit-cost-intelligence-observation:${stableSnapshotHash({
    unitCostIntelligenceObservationKey: input.unitCostIntelligenceObservationKey,
    unitCostIntelligenceCategory: input.unitCostIntelligenceCategory,
    ...
    unitCostReferenceIds: getUnitCostReferenceIds(input),
...
function validateArtifacts(input: BuildUnitCostIntelligenceObservationInput, warnings: string[]): void {
...
function validateInput(input: BuildUnitCostIntelligenceObservationInput): string[] {
  ...
  if (!hasValue(input.unitCostIntelligenceObservationKey)) {
    warnings.push("unitCostIntelligenceObservationKey is required.");
  }
  if (!hasValue(input.unitCostIntelligenceCategory)) warnings.push("unitCostIntelligenceCategory is required.");
  if (!isSupportedUnitCostIntelligenceCategory(input.unitCostIntelligenceCategory)) {
    warnings.push("unitCostIntelligenceCategory must be a supported unit cost intelligence category.");
  }
...
export function buildUnitCostIntelligenceObservation(
  input: BuildUnitCostIntelligenceObservationInput,
): BuildUnitCostIntelligenceObservationResult {
  const warnings = validateInput(input);
  if (warnings.length > 0 || !input.auditContract) {
    return {
      unitCostIntelligenceObservation: null,
      skipped: true,
      warnings,
    };
  }
  ...
  return {
    unitCostIntelligenceObservation: {
      unitCostIntelligenceObservationId: buildUnitCostIntelligenceObservationId(input),
      unitCostIntelligenceObservationKey: input.unitCostIntelligenceObservationKey,
      unitCostIntelligenceCategory: input.unitCostIntelligenceCategory,
