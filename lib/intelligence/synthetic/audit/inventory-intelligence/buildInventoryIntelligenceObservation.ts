...
export type SyntheticInventoryIntelligenceCategory =
  | "inventory_balance"
  | "inventory_movement"
  | "inventory_quantity_change"
  | "inventory_valuation_candidate"
  | "inventory_turnover_candidate"
  | "inventory_aging_candidate"
  | "inventory_reserve_relationship"
  | "inventory_obsolescence_candidate"
  | "inventory_cost_change"
  | "inventory_reconciliation_risk"
  | "inventory_cutoff_candidate"
  | "inventory_financial_statement_relationship";

export const SYNTHETIC_INVENTORY_INTELLIGENCE_CATEGORIES: SyntheticInventoryIntelligenceCategory[] = [
  "inventory_balance",
  "inventory_movement",
  "inventory_quantity_change",
  "inventory_valuation_candidate",
...
export interface BuildInventoryIntelligenceObservationInput {
  auditContract: SyntheticAuditContract | null;
  inventoryIntelligenceObservationKey: string;
  inventoryIntelligenceCategory: SyntheticInventoryIntelligenceCategory;
...
export interface SyntheticInventoryIntelligenceObservation {
  inventoryIntelligenceObservationId: string;
  inventoryIntelligenceObservationKey: string;
  inventoryIntelligenceCategory: SyntheticInventoryIntelligenceCategory;
  companyId: string;
...
export interface BuildInventoryIntelligenceObservationResult {
  inventoryIntelligenceObservation: SyntheticInventoryIntelligenceObservation | null;
  skipped: boolean;
  warnings: string[];
}
...
function isSupportedInventoryIntelligenceCategory(category: SyntheticInventoryIntelligenceCategory): boolean {
  return SYNTHETIC_INVENTORY_INTELLIGENCE_CATEGORIES.includes(category);
}
...
function getAuditCandidates(input: BuildInventoryIntelligenceObservationInput): SyntheticAuditCandidate[] {
  return input.auditCandidates ?? [];
}

function getAuditEvidencePackages(input: BuildInventoryIntelligenceObservationInput): SyntheticAuditEvidencePackage[] {
  return input.auditEvidencePackages ?? [];
}

function getAuditFindings(input: BuildInventoryIntelligenceObservationInput): SyntheticAuditFinding[] {
  return input.auditFindings ?? [];
}

function getAuditConfidencePackages(input: BuildInventoryIntelligenceObservationInput): SyntheticAuditConfidence[] {
  return input.auditConfidencePackages ?? [];
}

function getAuditSurfaces(input: BuildInventoryIntelligenceObservationInput): SyntheticAuditSurface[] {
  return input.auditSurfaces ?? [];
}

function getAuditWatchlists(input: BuildInventoryIntelligenceObservationInput): SyntheticAuditWatchlist[] {
  return input.auditWatchlists ?? [];
}

function getAuditBriefings(input: BuildInventoryIntelligenceObservationInput): SyntheticAuditBriefing[] {
  return input.auditBriefings ?? [];
}

function getAllAuditArtifacts(input: BuildInventoryIntelligenceObservationInput): AuditArtifact[] {
  return [
...
function getPackagedAuditArtifacts(
  input: BuildInventoryIntelligenceObservationInput,
): Array<
  | SyntheticAuditEvidencePackage
  | SyntheticAuditFinding
  | SyntheticAuditConfidence
...
function getEvidenceReferenceIds(input: BuildInventoryIntelligenceObservationInput): string[] {
  return uniqueStable([
...
function getSourceReferenceIds(input: BuildInventoryIntelligenceObservationInput): string[] {
  return uniqueStable([
...
function getLineageReferenceIds(input: BuildInventoryIntelligenceObservationInput): string[] {
  return uniqueStable([
...
function getInventoryReferenceIds(input: BuildInventoryIntelligenceObservationInput): string[] {
  return uniqueStable([
...
function buildInventoryIntelligenceObservationId(input: BuildInventoryIntelligenceObservationInput): string {
  return `synthetic-inventory-intelligence-observation:${stableSnapshotHash({
    inventoryIntelligenceObservationKey: input.inventoryIntelligenceObservationKey,
    inventoryIntelligenceCategory: input.inventoryIntelligenceCategory,
    ...
    inventoryReferenceIds: getInventoryReferenceIds(input),
...
function validateArtifacts(input: BuildInventoryIntelligenceObservationInput, warnings: string[]): void {
...
function validateInput(input: BuildInventoryIntelligenceObservationInput): string[] {
  ...
  if (!hasValue(input.inventoryIntelligenceObservationKey)) {
    warnings.push("inventoryIntelligenceObservationKey is required.");
  }
  if (!hasValue(input.inventoryIntelligenceCategory)) warnings.push("inventoryIntelligenceCategory is required.");
  if (!isSupportedInventoryIntelligenceCategory(input.inventoryIntelligenceCategory)) {
    warnings.push("inventoryIntelligenceCategory must be a supported inventory intelligence category.");
  }
...
export function buildInventoryIntelligenceObservation(
  input: BuildInventoryIntelligenceObservationInput,
): BuildInventoryIntelligenceObservationResult {
  const warnings = validateInput(input);
  if (warnings.length > 0 || !input.auditContract) {
    return {
      inventoryIntelligenceObservation: null,
      skipped: true,
      warnings,
    };
  }
  ...
  return {
    inventoryIntelligenceObservation: {
      inventoryIntelligenceObservationId: buildInventoryIntelligenceObservationId(input),
      inventoryIntelligenceObservationKey: input.inventoryIntelligenceObservationKey,
      inventoryIntelligenceCategory: input.inventoryIntelligenceCategory,
