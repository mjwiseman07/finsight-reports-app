...
export type SyntheticRecurringPatternCategory =
  | "recurring_vendor_invoice"
  | "recurring_utility_expense"
  | "recurring_payroll_activity"
  | "recurring_journal_entry"
  | "recurring_depreciation_entry"
  | "recurring_amortization_entry"
  | "recurring_tax_remittance"
  | "recurring_lease_payment"
  | "recurring_debt_payment"
  | "recurring_customer_billing"
  | "recurring_inventory_adjustment"
  | "recurring_reserve_update"
  | "recurring_reconciliation_activity";

export const SYNTHETIC_RECURRING_PATTERN_CATEGORIES: SyntheticRecurringPatternCategory[] = [
  "recurring_vendor_invoice",
  "recurring_utility_expense",
  "recurring_payroll_activity",
  "recurring_journal_entry",
...
export interface BuildRecurringPatternObservationInput {
  auditContract: SyntheticAuditContract | null;
  recurringPatternObservationKey: string;
  recurringPatternCategory: SyntheticRecurringPatternCategory;
...
export interface SyntheticRecurringPatternObservation {
  recurringPatternObservationId: string;
  recurringPatternObservationKey: string;
  recurringPatternCategory: SyntheticRecurringPatternCategory;
  companyId: string;
...
export interface BuildRecurringPatternObservationResult {
  recurringPatternObservation: SyntheticRecurringPatternObservation | null;
  skipped: boolean;
  warnings: string[];
}
...
function isSupportedRecurringPatternCategory(category: SyntheticRecurringPatternCategory): boolean {
  return SYNTHETIC_RECURRING_PATTERN_CATEGORIES.includes(category);
}
...
function getAuditCandidates(input: BuildRecurringPatternObservationInput): SyntheticAuditCandidate[] {
  return input.auditCandidates ?? [];
}

function getAuditEvidencePackages(input: BuildRecurringPatternObservationInput): SyntheticAuditEvidencePackage[] {
  return input.auditEvidencePackages ?? [];
}

function getAuditFindings(input: BuildRecurringPatternObservationInput): SyntheticAuditFinding[] {
  return input.auditFindings ?? [];
}

function getAuditConfidencePackages(input: BuildRecurringPatternObservationInput): SyntheticAuditConfidence[] {
  return input.auditConfidencePackages ?? [];
}

function getAuditSurfaces(input: BuildRecurringPatternObservationInput): SyntheticAuditSurface[] {
  return input.auditSurfaces ?? [];
}

function getAuditWatchlists(input: BuildRecurringPatternObservationInput): SyntheticAuditWatchlist[] {
  return input.auditWatchlists ?? [];
}

function getAuditBriefings(input: BuildRecurringPatternObservationInput): SyntheticAuditBriefing[] {
  return input.auditBriefings ?? [];
}

function getAllAuditArtifacts(input: BuildRecurringPatternObservationInput): AuditArtifact[] {
  return [
...
function getPackagedAuditArtifacts(
  input: BuildRecurringPatternObservationInput,
): Array<
  | SyntheticAuditEvidencePackage
  | SyntheticAuditFinding
  | SyntheticAuditConfidence
...
function getEvidenceReferenceIds(input: BuildRecurringPatternObservationInput): string[] {
  return uniqueStable([
...
function getSourceReferenceIds(input: BuildRecurringPatternObservationInput): string[] {
  return uniqueStable([
...
function getLineageReferenceIds(input: BuildRecurringPatternObservationInput): string[] {
  return uniqueStable([
...
function buildRecurringPatternObservationId(input: BuildRecurringPatternObservationInput): string {
  return `synthetic-recurring-pattern-observation:${stableSnapshotHash({
    recurringPatternObservationKey: input.recurringPatternObservationKey,
    recurringPatternCategory: input.recurringPatternCategory,
...
function validateArtifacts(input: BuildRecurringPatternObservationInput, warnings: string[]): void {
...
function validateInput(input: BuildRecurringPatternObservationInput): string[] {
  ...
  if (!hasValue(input.recurringPatternObservationKey)) warnings.push("recurringPatternObservationKey is required.");
  if (!hasValue(input.recurringPatternCategory)) warnings.push("recurringPatternCategory is required.");
  if (!isSupportedRecurringPatternCategory(input.recurringPatternCategory)) {
    warnings.push("recurringPatternCategory must be a supported recurring pattern category.");
  }
...
export function buildRecurringPatternObservation(
  input: BuildRecurringPatternObservationInput,
): BuildRecurringPatternObservationResult {
  const warnings = validateInput(input);
  if (warnings.length > 0 || !input.auditContract) {
    return {
      recurringPatternObservation: null,
      skipped: true,
      warnings,
    };
  }
  ...
  return {
    recurringPatternObservation: {
      recurringPatternObservationId: buildRecurringPatternObservationId(input),
      recurringPatternObservationKey: input.recurringPatternObservationKey,
      recurringPatternCategory: input.recurringPatternCategory,
