...
export type SyntheticReconciliationCategory =
  | "stale_reconciliation"
  | "aged_reconciling_item"
  | "recurring_reconciling_item"
  | "unresolved_difference"
  | "growing_difference"
  | "unexplained_difference"
  | "cash_reconciliation_issue"
  | "bank_reconciliation_issue"
  | "inventory_reconciliation_issue"
  | "intercompany_reconciliation_issue"
  | "tax_reconciliation_issue"
  | "reserve_reconciliation_issue"
  | "close_readiness_reconciliation_risk";

export const SYNTHETIC_RECONCILIATION_CATEGORIES: SyntheticReconciliationCategory[] = [
  "stale_reconciliation",
  "aged_reconciling_item",
  "recurring_reconciling_item",
  "unresolved_difference",
...
export interface BuildReconciliationObservationInput {
  auditContract: SyntheticAuditContract | null;
  reconciliationObservationKey: string;
  reconciliationCategory: SyntheticReconciliationCategory;
...
export interface SyntheticReconciliationObservation {
  reconciliationObservationId: string;
  reconciliationObservationKey: string;
  reconciliationCategory: SyntheticReconciliationCategory;
  companyId: string;
...
export interface BuildReconciliationObservationResult {
  reconciliationObservation: SyntheticReconciliationObservation | null;
  skipped: boolean;
  warnings: string[];
}
...
function isSupportedReconciliationCategory(category: SyntheticReconciliationCategory): boolean {
  return SYNTHETIC_RECONCILIATION_CATEGORIES.includes(category);
}
...
function getAuditCandidates(input: BuildReconciliationObservationInput): SyntheticAuditCandidate[] {
  return input.auditCandidates ?? [];
}

function getAuditEvidencePackages(input: BuildReconciliationObservationInput): SyntheticAuditEvidencePackage[] {
  return input.auditEvidencePackages ?? [];
}

function getAuditFindings(input: BuildReconciliationObservationInput): SyntheticAuditFinding[] {
  return input.auditFindings ?? [];
}

function getAuditConfidencePackages(input: BuildReconciliationObservationInput): SyntheticAuditConfidence[] {
  return input.auditConfidencePackages ?? [];
}

function getAuditSurfaces(input: BuildReconciliationObservationInput): SyntheticAuditSurface[] {
  return input.auditSurfaces ?? [];
}

function getAuditWatchlists(input: BuildReconciliationObservationInput): SyntheticAuditWatchlist[] {
  return input.auditWatchlists ?? [];
}

function getAuditBriefings(input: BuildReconciliationObservationInput): SyntheticAuditBriefing[] {
  return input.auditBriefings ?? [];
}

function getAllAuditArtifacts(input: BuildReconciliationObservationInput): AuditArtifact[] {
  return [
...
function getPackagedAuditArtifacts(
  input: BuildReconciliationObservationInput,
): Array<
  | SyntheticAuditEvidencePackage
  | SyntheticAuditFinding
  | SyntheticAuditConfidence
...
function getEvidenceReferenceIds(input: BuildReconciliationObservationInput): string[] {
  return uniqueStable([
...
function getSourceReferenceIds(input: BuildReconciliationObservationInput): string[] {
  return uniqueStable([
...
function getLineageReferenceIds(input: BuildReconciliationObservationInput): string[] {
  return uniqueStable([
...
function getReconciliationReferenceIds(input: BuildReconciliationObservationInput): string[] {
  return uniqueStable([
    ...(input.auditContract?.observationMetadata?.relatedReconciliationIds ?? []),
    ...(input.auditContract?.evidence.supportingReconciliationIds ?? []),
    ...getAllAuditArtifacts(input).flatMap((artifact) => artifact.observationMetadata?.relatedReconciliationIds ?? []),
    ...getAllAuditArtifacts(input).flatMap((artifact) => artifact.evidence.supportingReconciliationIds),
  ]);
}

function buildReconciliationObservationId(input: BuildReconciliationObservationInput): string {
  return `synthetic-reconciliation-observation:${stableSnapshotHash({
    reconciliationObservationKey: input.reconciliationObservationKey,
    reconciliationCategory: input.reconciliationCategory,
    ...
    reconciliationReferenceIds: getReconciliationReferenceIds(input),
...
function validateArtifacts(input: BuildReconciliationObservationInput, warnings: string[]): void {
...
function validateInput(input: BuildReconciliationObservationInput): string[] {
  ...
  if (!hasValue(input.reconciliationObservationKey)) warnings.push("reconciliationObservationKey is required.");
  if (!hasValue(input.reconciliationCategory)) warnings.push("reconciliationCategory is required.");
  if (!isSupportedReconciliationCategory(input.reconciliationCategory)) {
    warnings.push("reconciliationCategory must be a supported reconciliation category.");
  }
...
export function buildReconciliationObservation(
  input: BuildReconciliationObservationInput,
): BuildReconciliationObservationResult {
  ...
  return {
    reconciliationObservation: {
      reconciliationObservationId: buildReconciliationObservationId(input),
      reconciliationObservationKey: input.reconciliationObservationKey,
      reconciliationCategory: input.reconciliationCategory,
