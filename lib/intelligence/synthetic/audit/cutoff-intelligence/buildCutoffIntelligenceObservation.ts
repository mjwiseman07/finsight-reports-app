...
export type SyntheticCutoffIntelligenceCategory =
  | "revenue_cutoff_candidate"
  | "expense_cutoff_candidate"
  | "inventory_cutoff_candidate"
  | "ar_cutoff_candidate"
  | "ap_cutoff_candidate"
  | "cash_cutoff_candidate"
  | "deferred_revenue_cutoff_candidate"
  | "shipment_timing_candidate"
  | "invoice_timing_candidate"
  | "receipt_timing_candidate"
  | "period_end_activity_candidate"
  | "cutoff_reconciliation_risk";

export const SYNTHETIC_CUTOFF_INTELLIGENCE_CATEGORIES: SyntheticCutoffIntelligenceCategory[] = [
  "revenue_cutoff_candidate",
  "expense_cutoff_candidate",
  "inventory_cutoff_candidate",
  "ar_cutoff_candidate",
...
export interface BuildCutoffIntelligenceObservationInput {
  auditContract: SyntheticAuditContract | null;
  cutoffIntelligenceObservationKey: string;
  cutoffIntelligenceCategory: SyntheticCutoffIntelligenceCategory;
...
export interface SyntheticCutoffIntelligenceObservation {
  cutoffIntelligenceObservationId: string;
  cutoffIntelligenceObservationKey: string;
  cutoffIntelligenceCategory: SyntheticCutoffIntelligenceCategory;
  companyId: string;
...
export interface BuildCutoffIntelligenceObservationResult {
  cutoffIntelligenceObservation: SyntheticCutoffIntelligenceObservation | null;
  skipped: boolean;
  warnings: string[];
}
...
function isSupportedCutoffIntelligenceCategory(category: SyntheticCutoffIntelligenceCategory): boolean {
  return SYNTHETIC_CUTOFF_INTELLIGENCE_CATEGORIES.includes(category);
}
...
function getAuditCandidates(input: BuildCutoffIntelligenceObservationInput): SyntheticAuditCandidate[] {
  return input.auditCandidates ?? [];
}

function getAuditEvidencePackages(input: BuildCutoffIntelligenceObservationInput): SyntheticAuditEvidencePackage[] {
  return input.auditEvidencePackages ?? [];
}

function getAuditFindings(input: BuildCutoffIntelligenceObservationInput): SyntheticAuditFinding[] {
  return input.auditFindings ?? [];
}

function getAuditConfidencePackages(input: BuildCutoffIntelligenceObservationInput): SyntheticAuditConfidence[] {
  return input.auditConfidencePackages ?? [];
}

function getAuditSurfaces(input: BuildCutoffIntelligenceObservationInput): SyntheticAuditSurface[] {
  return input.auditSurfaces ?? [];
}

function getAuditWatchlists(input: BuildCutoffIntelligenceObservationInput): SyntheticAuditWatchlist[] {
  return input.auditWatchlists ?? [];
}

function getAuditBriefings(input: BuildCutoffIntelligenceObservationInput): SyntheticAuditBriefing[] {
  return input.auditBriefings ?? [];
}

function getAllAuditArtifacts(input: BuildCutoffIntelligenceObservationInput): AuditArtifact[] {
  return [
...
function getPackagedAuditArtifacts(
  input: BuildCutoffIntelligenceObservationInput,
): Array<
  | SyntheticAuditEvidencePackage
  | SyntheticAuditFinding
  | SyntheticAuditConfidence
...
function getEvidenceReferenceIds(input: BuildCutoffIntelligenceObservationInput): string[] {
  return uniqueStable([
...
function getSourceReferenceIds(input: BuildCutoffIntelligenceObservationInput): string[] {
  return uniqueStable([
...
function getLineageReferenceIds(input: BuildCutoffIntelligenceObservationInput): string[] {
  return uniqueStable([
...
function getCutoffReferenceIds(input: BuildCutoffIntelligenceObservationInput): string[] {
  return uniqueStable([
...
function buildCutoffIntelligenceObservationId(input: BuildCutoffIntelligenceObservationInput): string {
  return `synthetic-cutoff-intelligence-observation:${stableSnapshotHash({
    cutoffIntelligenceObservationKey: input.cutoffIntelligenceObservationKey,
    cutoffIntelligenceCategory: input.cutoffIntelligenceCategory,
    ...
    cutoffReferenceIds: getCutoffReferenceIds(input),
...
function validateArtifacts(input: BuildCutoffIntelligenceObservationInput, warnings: string[]): void {
...
function validateInput(input: BuildCutoffIntelligenceObservationInput): string[] {
  ...
  if (!hasValue(input.cutoffIntelligenceObservationKey)) warnings.push("cutoffIntelligenceObservationKey is required.");
  if (!hasValue(input.cutoffIntelligenceCategory)) warnings.push("cutoffIntelligenceCategory is required.");
  if (!isSupportedCutoffIntelligenceCategory(input.cutoffIntelligenceCategory)) {
    warnings.push("cutoffIntelligenceCategory must be a supported cutoff intelligence category.");
  }
...
export function buildCutoffIntelligenceObservation(
  input: BuildCutoffIntelligenceObservationInput,
): BuildCutoffIntelligenceObservationResult {
  const warnings = validateInput(input);
  if (warnings.length > 0 || !input.auditContract) {
    return {
      cutoffIntelligenceObservation: null,
      skipped: true,
      warnings,
    };
  }
  ...
  return {
    cutoffIntelligenceObservation: {
      cutoffIntelligenceObservationId: buildCutoffIntelligenceObservationId(input),
      cutoffIntelligenceObservationKey: input.cutoffIntelligenceObservationKey,
      cutoffIntelligenceCategory: input.cutoffIntelligenceCategory,
