...
export type SyntheticJournalTestingCategory =
  | "round_dollar_entry"
  | "manual_journal_entry"
  | "period_end_journal_entry"
  | "after_hours_journal_entry"
  | "weekend_journal_entry"
  | "privileged_user_journal_entry"
  | "dormant_account_journal_entry"
  | "unusual_account_combination"
  | "statistical_outlier_candidate"
  | "revenue_impacting_entry"
  | "reserve_impacting_entry"
  | "tax_impacting_entry"
  | "cash_impacting_entry"
  | "management_override_candidate";

export const SYNTHETIC_JOURNAL_TESTING_CATEGORIES: SyntheticJournalTestingCategory[] = [
  "round_dollar_entry",
  "manual_journal_entry",
  "period_end_journal_entry",
  "after_hours_journal_entry",
...
export interface BuildJournalTestingObservationInput {
  auditContract: SyntheticAuditContract | null;
  journalTestingObservationKey: string;
  journalTestingCategory: SyntheticJournalTestingCategory;
...
export interface SyntheticJournalTestingObservation {
  journalTestingObservationId: string;
  journalTestingObservationKey: string;
  journalTestingCategory: SyntheticJournalTestingCategory;
  companyId: string;
  ...
  journalEntryReferenceIds: string[];
...
export interface BuildJournalTestingObservationResult {
  journalTestingObservation: SyntheticJournalTestingObservation | null;
  skipped: boolean;
  warnings: string[];
}
...
function isSupportedJournalTestingCategory(category: SyntheticJournalTestingCategory): boolean {
  return SYNTHETIC_JOURNAL_TESTING_CATEGORIES.includes(category);
}
...
function getAuditCandidates(input: BuildJournalTestingObservationInput): SyntheticAuditCandidate[] {
  return input.auditCandidates ?? [];
}

function getAuditEvidencePackages(input: BuildJournalTestingObservationInput): SyntheticAuditEvidencePackage[] {
  return input.auditEvidencePackages ?? [];
}

function getAuditFindings(input: BuildJournalTestingObservationInput): SyntheticAuditFinding[] {
  return input.auditFindings ?? [];
}

function getAuditConfidencePackages(input: BuildJournalTestingObservationInput): SyntheticAuditConfidence[] {
  return input.auditConfidencePackages ?? [];
}

function getAuditSurfaces(input: BuildJournalTestingObservationInput): SyntheticAuditSurface[] {
  return input.auditSurfaces ?? [];
}

function getAuditWatchlists(input: BuildJournalTestingObservationInput): SyntheticAuditWatchlist[] {
  return input.auditWatchlists ?? [];
}

function getAuditBriefings(input: BuildJournalTestingObservationInput): SyntheticAuditBriefing[] {
  return input.auditBriefings ?? [];
}

function getAllAuditArtifacts(input: BuildJournalTestingObservationInput): AuditArtifact[] {
  return [
...
function getPackagedAuditArtifacts(
  input: BuildJournalTestingObservationInput,
): Array<
  | SyntheticAuditEvidencePackage
  | SyntheticAuditFinding
  | SyntheticAuditConfidence
...
function getEvidenceReferenceIds(input: BuildJournalTestingObservationInput): string[] {
  return uniqueStable([
...
function getSourceReferenceIds(input: BuildJournalTestingObservationInput): string[] {
  return uniqueStable([
...
function getLineageReferenceIds(input: BuildJournalTestingObservationInput): string[] {
  return uniqueStable([
...
function getJournalEntryReferenceIds(input: BuildJournalTestingObservationInput): string[] {
  return uniqueStable([
    ...(input.auditContract?.observationMetadata?.relatedJournalEntryIds ?? []),
    ...(input.auditContract?.evidence.supportingJournalEntryIds ?? []),
    ...getAllAuditArtifacts(input).flatMap((artifact) => artifact.observationMetadata?.relatedJournalEntryIds ?? []),
    ...getAllAuditArtifacts(input).flatMap((artifact) => artifact.evidence.supportingJournalEntryIds),
  ]);
}

function buildJournalTestingObservationId(input: BuildJournalTestingObservationInput): string {
  return `synthetic-journal-testing-observation:${stableSnapshotHash({
    journalTestingObservationKey: input.journalTestingObservationKey,
    journalTestingCategory: input.journalTestingCategory,
    ...
    journalEntryReferenceIds: getJournalEntryReferenceIds(input),
...
function validateArtifacts(input: BuildJournalTestingObservationInput, warnings: string[]): void {
...
function validateInput(input: BuildJournalTestingObservationInput): string[] {
  ...
  if (!hasValue(input.journalTestingObservationKey)) warnings.push("journalTestingObservationKey is required.");
  if (!hasValue(input.journalTestingCategory)) warnings.push("journalTestingCategory is required.");
  if (!isSupportedJournalTestingCategory(input.journalTestingCategory)) {
    warnings.push("journalTestingCategory must be a supported journal testing category.");
  }
...
export function buildJournalTestingObservation(
  input: BuildJournalTestingObservationInput,
): BuildJournalTestingObservationResult {
  const warnings = validateInput(input);
  if (warnings.length > 0 || !input.auditContract) {
    return {
      journalTestingObservation: null,
      skipped: true,
      warnings,
    };
  }
  ...
  return {
    journalTestingObservation: {
      journalTestingObservationId: buildJournalTestingObservationId(input),
      journalTestingObservationKey: input.journalTestingObservationKey,
      journalTestingCategory: input.journalTestingCategory,
