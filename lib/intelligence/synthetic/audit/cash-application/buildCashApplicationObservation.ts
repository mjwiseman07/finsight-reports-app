...
export type SyntheticCashApplicationCategory =
  | "customer_payment"
  | "cash_receipt"
  | "bank_deposit"
  | "unapplied_cash_candidate"
  | "partially_applied_cash_candidate"
  | "unidentified_receipt"
  | "deposit_exception_candidate"
  | "receipt_timing_exception"
  | "invoice_application_candidate"
  | "lockbox_receipt"
  | "ach_receipt"
  | "wire_receipt"
  | "cash_application_risk";

export const SYNTHETIC_CASH_APPLICATION_CATEGORIES: SyntheticCashApplicationCategory[] = [
  "customer_payment",
  "cash_receipt",
  "bank_deposit",
  "unapplied_cash_candidate",
...
export interface BuildCashApplicationObservationInput {
  auditContract: SyntheticAuditContract | null;
  cashApplicationObservationKey: string;
  cashApplicationCategory: SyntheticCashApplicationCategory;
...
export interface SyntheticCashApplicationObservation {
  cashApplicationObservationId: string;
  cashApplicationObservationKey: string;
  cashApplicationCategory: SyntheticCashApplicationCategory;
  companyId: string;
  ...
  cashApplicationReferenceIds: string[];
...
export interface BuildCashApplicationObservationResult {
  cashApplicationObservation: SyntheticCashApplicationObservation | null;
  skipped: boolean;
  warnings: string[];
}
...
function isSupportedCashApplicationCategory(category: SyntheticCashApplicationCategory): boolean {
  return SYNTHETIC_CASH_APPLICATION_CATEGORIES.includes(category);
}
...
function getAuditCandidates(input: BuildCashApplicationObservationInput): SyntheticAuditCandidate[] {
  return input.auditCandidates ?? [];
}

function getAuditEvidencePackages(input: BuildCashApplicationObservationInput): SyntheticAuditEvidencePackage[] {
  return input.auditEvidencePackages ?? [];
}

function getAuditFindings(input: BuildCashApplicationObservationInput): SyntheticAuditFinding[] {
  return input.auditFindings ?? [];
}

function getAuditConfidencePackages(input: BuildCashApplicationObservationInput): SyntheticAuditConfidence[] {
  return input.auditConfidencePackages ?? [];
}

function getAuditSurfaces(input: BuildCashApplicationObservationInput): SyntheticAuditSurface[] {
  return input.auditSurfaces ?? [];
}

function getAuditWatchlists(input: BuildCashApplicationObservationInput): SyntheticAuditWatchlist[] {
  return input.auditWatchlists ?? [];
}

function getAuditBriefings(input: BuildCashApplicationObservationInput): SyntheticAuditBriefing[] {
  return input.auditBriefings ?? [];
}

function getAllAuditArtifacts(input: BuildCashApplicationObservationInput): AuditArtifact[] {
  return [
...
function getPackagedAuditArtifacts(
  input: BuildCashApplicationObservationInput,
): Array<
  | SyntheticAuditEvidencePackage
  | SyntheticAuditFinding
  | SyntheticAuditConfidence
...
function getEvidenceReferenceIds(input: BuildCashApplicationObservationInput): string[] {
  return uniqueStable([
...
function getSourceReferenceIds(input: BuildCashApplicationObservationInput): string[] {
  return uniqueStable([
...
function getLineageReferenceIds(input: BuildCashApplicationObservationInput): string[] {
  return uniqueStable([
...
function getCashApplicationReferenceIds(input: BuildCashApplicationObservationInput): string[] {
  return uniqueStable([
...
function buildCashApplicationObservationId(input: BuildCashApplicationObservationInput): string {
  return `synthetic-cash-application-observation:${stableSnapshotHash({
    cashApplicationObservationKey: input.cashApplicationObservationKey,
    cashApplicationCategory: input.cashApplicationCategory,
    ...
    cashApplicationReferenceIds: getCashApplicationReferenceIds(input),
...
function validateArtifacts(input: BuildCashApplicationObservationInput, warnings: string[]): void {
...
function validateInput(input: BuildCashApplicationObservationInput): string[] {
  ...
  if (!hasValue(input.cashApplicationObservationKey)) warnings.push("cashApplicationObservationKey is required.");
  if (!hasValue(input.cashApplicationCategory)) warnings.push("cashApplicationCategory is required.");
  if (!isSupportedCashApplicationCategory(input.cashApplicationCategory)) {
    warnings.push("cashApplicationCategory must be a supported cash application category.");
  }
...
export function buildCashApplicationObservation(
  input: BuildCashApplicationObservationInput,
): BuildCashApplicationObservationResult {
  const warnings = validateInput(input);
  if (warnings.length > 0 || !input.auditContract) {
    return {
      cashApplicationObservation: null,
      skipped: true,
      warnings,
    };
  }
  ...
  return {
    cashApplicationObservation: {
      cashApplicationObservationId: buildCashApplicationObservationId(input),
      cashApplicationObservationKey: input.cashApplicationObservationKey,
      cashApplicationCategory: input.cashApplicationCategory,
