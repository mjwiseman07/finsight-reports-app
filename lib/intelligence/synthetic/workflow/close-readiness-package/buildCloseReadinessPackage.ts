...
export type SyntheticCloseReadinessPackageCategory =
  | "close_readiness_package"
  | "close_schedule_package"
  | "close_tie_out_package"
  | "close_evidence_package"
  | "close_trust_package"
  | "close_period_end_package"
  | "close_wave_one_context_package";

export const SYNTHETIC_CLOSE_READINESS_PACKAGE_CATEGORIES: SyntheticCloseReadinessPackageCategory[] = [
  "close_readiness_package",
  "close_schedule_package",
  "close_tie_out_package",
  "close_evidence_package",
...
export interface SyntheticCloseReadinessPackageIsolationDimension {
  required: boolean;
  referenceIds: string[];
}

export interface SyntheticCloseReadinessForwardCompatibleObservation {
...
export interface BuildCloseReadinessPackageInput {
  auditContract: SyntheticAuditContract | null;
  packageKey: string;
  packageCategory: SyntheticCloseReadinessPackageCategory;
  ...
  healthcarePpdObservations?: SyntheticCloseReadinessForwardCompatibleObservation[];
  payrollObservations?: SyntheticCloseReadinessForwardCompatibleObservation[];
  methodologyObservations?: SyntheticCloseReadinessForwardCompatibleObservation[];
...
export interface SyntheticCloseReadinessPackage {
  packageId: string;
  packageKey: string;
  packageCategory: SyntheticCloseReadinessPackageCategory;
  companyId: string;
  ...
  customerIsolation: SyntheticCloseReadinessPackageIsolationDimension;
  firmIsolation: SyntheticCloseReadinessPackageIsolationDimension;
  clientIsolation: SyntheticCloseReadinessPackageIsolationDimension;
  evidenceReferenceIds: string[];
  sourceReferenceIds: string[];
  lineageReferenceIds: string[];
  auditContractReferenceIds: string[];
...
export interface BuildCloseReadinessPackageResult {
  closeReadinessPackage: SyntheticCloseReadinessPackage | null;
  skipped: boolean;
  warnings: string[];
}
...
function isSupportedPackageCategory(category: SyntheticCloseReadinessPackageCategory): boolean {
  return SYNTHETIC_CLOSE_READINESS_PACKAGE_CATEGORIES.includes(category);
}

function buildCustomerIsolation(scope: SyntheticAuditScope): SyntheticCloseReadinessPackageIsolationDimension {
  return { required: scope.customerIsolationRequired, referenceIds: uniqueStable([scope.companyId, ...scope.isolationBoundaryIds]) };
}

function buildFirmIsolation(scope: SyntheticAuditScope): SyntheticCloseReadinessPackageIsolationDimension {
  return {
...
function buildClientIsolation(scope: SyntheticAuditScope): SyntheticCloseReadinessPackageIsolationDimension {
  return {
...
function getDomainObservations(input: BuildCloseReadinessPackageInput): object[] {
  return [
...
function getAuditArtifacts(input: BuildCloseReadinessPackageInput): AuditArtifact[] {
  return [
...
function getPackagedAuditArtifacts(
  input: BuildCloseReadinessPackageInput,
): Array<
  | SyntheticAuditEvidencePackage
  | SyntheticAuditFinding
  | SyntheticAuditConfidence
...
function getUpstreamPackages(input: BuildCloseReadinessPackageInput): UpstreamPackage[] {
  return [
...
function getAuditContractReferenceIds(input: BuildCloseReadinessPackageInput): string[] {
...
function getReferenceIds(input: BuildCloseReadinessPackageInput, singularName: string, arrayName: string): string[] {
  return uniqueStable([
...
function getEvidenceReferenceIds(input: BuildCloseReadinessPackageInput): string[] {
  return uniqueStable([
...
function getSourceReferenceIds(input: BuildCloseReadinessPackageInput): string[] {
  return uniqueStable([
...
function getLineageReferenceIds(input: BuildCloseReadinessPackageInput): string[] {
  return uniqueStable([
...
function getCloseReadinessObservationIds(input: BuildCloseReadinessPackageInput) {
  return {
...
function getUpstreamObservationIds(input: BuildCloseReadinessPackageInput): string[] {
  const observationIds = getCloseReadinessObservationIds(input);
  return uniqueStable([
...
function getMaterialityMetadata(input: BuildCloseReadinessPackageInput): SyntheticAuditMaterialityCompatibility[] {
...
function buildPackageId(input: BuildCloseReadinessPackageInput): string {
  return `synthetic-close-readiness-package:${stableSnapshotHash({
    ...
    ...getCloseReadinessObservationIds(input),
...
function getForwardCompatibilityWarnings(input: BuildCloseReadinessPackageInput): string[] {
  return [
...
function validateInput(input: BuildCloseReadinessPackageInput): string[] {
...
export function buildCloseReadinessPackage(input: BuildCloseReadinessPackageInput): BuildCloseReadinessPackageResult {
  const fatalWarnings = validateInput(input);
  if (fatalWarnings.length > 0 || !input.auditContract) {
    return {
      closeReadinessPackage: null,
      skipped: true,
