...
export type SyntheticReconciliationReviewPackageCategory =
  | "reconciliation_review_package"
  | "cash_reconciliation_review_package"
  | "bank_reconciliation_review_package"
  | "tie_out_review_package"
  | "schedule_reconciliation_review_package"
  | "evidence_reconciliation_review_package"
  | "reconciliation_support_context_package";

export const SYNTHETIC_RECONCILIATION_REVIEW_PACKAGE_CATEGORIES: SyntheticReconciliationReviewPackageCategory[] = [
  "reconciliation_review_package",
  "cash_reconciliation_review_package",
  "bank_reconciliation_review_package",
  "tie_out_review_package",
...
export interface SyntheticReconciliationReviewPackageIsolationDimension {
  required: boolean;
  referenceIds: string[];
}

export interface SyntheticReconciliationReviewForwardCompatibleObservation {
...
export interface BuildReconciliationReviewPackageInput {
  auditContract: SyntheticAuditContract | null;
  packageKey: string;
  packageCategory: SyntheticReconciliationReviewPackageCategory;
  ...
  healthcarePpdObservations?: SyntheticReconciliationReviewForwardCompatibleObservation[];
  payrollObservations?: SyntheticReconciliationReviewForwardCompatibleObservation[];
  methodologyObservations?: SyntheticReconciliationReviewForwardCompatibleObservation[];
...
export interface SyntheticReconciliationReviewPackage {
  packageId: string;
  packageKey: string;
  packageCategory: SyntheticReconciliationReviewPackageCategory;
  companyId: string;
  ...
  customerIsolation: SyntheticReconciliationReviewPackageIsolationDimension;
  firmIsolation: SyntheticReconciliationReviewPackageIsolationDimension;
  clientIsolation: SyntheticReconciliationReviewPackageIsolationDimension;
  evidenceReferenceIds: string[];
  sourceReferenceIds: string[];
  lineageReferenceIds: string[];
  auditContractReferenceIds: string[];
...
export interface BuildReconciliationReviewPackageResult {
  reconciliationReviewPackage: SyntheticReconciliationReviewPackage | null;
  skipped: boolean;
  warnings: string[];
}
...
function isSupportedPackageCategory(category: SyntheticReconciliationReviewPackageCategory): boolean {
  return SYNTHETIC_RECONCILIATION_REVIEW_PACKAGE_CATEGORIES.includes(category);
}

function buildCustomerIsolation(scope: SyntheticAuditScope): SyntheticReconciliationReviewPackageIsolationDimension {
  return { required: scope.customerIsolationRequired, referenceIds: uniqueStable([scope.companyId, ...scope.isolationBoundaryIds]) };
}

function buildFirmIsolation(scope: SyntheticAuditScope): SyntheticReconciliationReviewPackageIsolationDimension {
  return {
...
function buildClientIsolation(scope: SyntheticAuditScope): SyntheticReconciliationReviewPackageIsolationDimension {
  return {
...
function getDomainObservations(input: BuildReconciliationReviewPackageInput): object[] {
  return [
...
function getAuditArtifacts(input: BuildReconciliationReviewPackageInput): AuditArtifact[] {
  return [
...
function getPackagedAuditArtifacts(
  input: BuildReconciliationReviewPackageInput,
): Array<
  | SyntheticAuditEvidencePackage
  | SyntheticAuditFinding
  | SyntheticAuditConfidence
...
function getUpstreamPackages(input: BuildReconciliationReviewPackageInput): UpstreamPackage[] {
  return [
...
function getAuditContractReferenceIds(input: BuildReconciliationReviewPackageInput): string[] {
...
function getReferenceIds(input: BuildReconciliationReviewPackageInput, singularName: string, arrayName: string): string[] {
  return uniqueStable([
...
function getEvidenceReferenceIds(input: BuildReconciliationReviewPackageInput): string[] {
  return uniqueStable([
...
function getSourceReferenceIds(input: BuildReconciliationReviewPackageInput): string[] {
  return uniqueStable([
...
function getLineageReferenceIds(input: BuildReconciliationReviewPackageInput): string[] {
  return uniqueStable([
...
function getReconciliationReviewObservationIds(input: BuildReconciliationReviewPackageInput) {
  return {
...
function getUpstreamObservationIds(input: BuildReconciliationReviewPackageInput): string[] {
  const observationIds = getReconciliationReviewObservationIds(input);
  return uniqueStable([
...
function getMaterialityMetadata(input: BuildReconciliationReviewPackageInput): SyntheticAuditMaterialityCompatibility[] {
...
function buildPackageId(input: BuildReconciliationReviewPackageInput): string {
  return `synthetic-reconciliation-review-package:${stableSnapshotHash({
    ...
    ...getReconciliationReviewObservationIds(input),
...
function getForwardCompatibilityWarnings(input: BuildReconciliationReviewPackageInput): string[] {
  return [
...
function validateInput(input: BuildReconciliationReviewPackageInput): string[] {
...
export function buildReconciliationReviewPackage(input: BuildReconciliationReviewPackageInput): BuildReconciliationReviewPackageResult {
  const fatalWarnings = validateInput(input);
  if (fatalWarnings.length > 0 || !input.auditContract) {
    return {
      reconciliationReviewPackage: null,
      skipped: true,
