...
export type SyntheticCloseHealthPackageCategory =
  | "close_health_package"
  | "close_flux_package"
  | "close_anomaly_package"
  | "close_cash_health_package"
  | "close_integrity_package"
  | "close_trust_health_package"
  | "close_readiness_context_package";

export const SYNTHETIC_CLOSE_HEALTH_PACKAGE_CATEGORIES: SyntheticCloseHealthPackageCategory[] = [
  "close_health_package",
  "close_flux_package",
  "close_anomaly_package",
  "close_cash_health_package",
...
export interface SyntheticCloseHealthPackageIsolationDimension {
  required: boolean;
  referenceIds: string[];
}

export interface SyntheticCloseHealthForwardCompatibleObservation {
...
export interface BuildCloseHealthPackageInput {
  auditContract: SyntheticAuditContract | null;
  packageKey: string;
  packageCategory: SyntheticCloseHealthPackageCategory;
  ...
  healthcarePpdObservations?: SyntheticCloseHealthForwardCompatibleObservation[];
  payrollObservations?: SyntheticCloseHealthForwardCompatibleObservation[];
  methodologyObservations?: SyntheticCloseHealthForwardCompatibleObservation[];
...
export interface SyntheticCloseHealthPackage {
  packageId: string;
  packageKey: string;
  packageCategory: SyntheticCloseHealthPackageCategory;
  companyId: string;
  ...
  customerIsolation: SyntheticCloseHealthPackageIsolationDimension;
  firmIsolation: SyntheticCloseHealthPackageIsolationDimension;
  clientIsolation: SyntheticCloseHealthPackageIsolationDimension;
  evidenceReferenceIds: string[];
  sourceReferenceIds: string[];
  lineageReferenceIds: string[];
  auditContractReferenceIds: string[];
...
export interface BuildCloseHealthPackageResult {
  closeHealthPackage: SyntheticCloseHealthPackage | null;
  skipped: boolean;
  warnings: string[];
}
...
function isSupportedPackageCategory(category: SyntheticCloseHealthPackageCategory): boolean {
  return SYNTHETIC_CLOSE_HEALTH_PACKAGE_CATEGORIES.includes(category);
}

function buildCustomerIsolation(scope: SyntheticAuditScope): SyntheticCloseHealthPackageIsolationDimension {
  return { required: scope.customerIsolationRequired, referenceIds: uniqueStable([scope.companyId, ...scope.isolationBoundaryIds]) };
}

function buildFirmIsolation(scope: SyntheticAuditScope): SyntheticCloseHealthPackageIsolationDimension {
  return {
...
function buildClientIsolation(scope: SyntheticAuditScope): SyntheticCloseHealthPackageIsolationDimension {
  return {
...
function getDomainObservations(input: BuildCloseHealthPackageInput): object[] {
  return [
...
function getAuditArtifacts(input: BuildCloseHealthPackageInput): AuditArtifact[] {
  return [
...
function getPackagedAuditArtifacts(
  input: BuildCloseHealthPackageInput,
): Array<
  | SyntheticAuditEvidencePackage
  | SyntheticAuditFinding
  | SyntheticAuditConfidence
...
function getUpstreamPackages(input: BuildCloseHealthPackageInput): UpstreamPackage[] {
  return [
...
function getAuditContractReferenceIds(input: BuildCloseHealthPackageInput): string[] {
...
function getReferenceIds(input: BuildCloseHealthPackageInput, singularName: string, arrayName: string): string[] {
  return uniqueStable([
...
function getEvidenceReferenceIds(input: BuildCloseHealthPackageInput): string[] {
  return uniqueStable([
...
function getSourceReferenceIds(input: BuildCloseHealthPackageInput): string[] {
  return uniqueStable([
...
function getLineageReferenceIds(input: BuildCloseHealthPackageInput): string[] {
  return uniqueStable([
...
function getCloseHealthObservationIds(input: BuildCloseHealthPackageInput) {
  return {
...
function getUpstreamObservationIds(input: BuildCloseHealthPackageInput): string[] {
  const observationIds = getCloseHealthObservationIds(input);
  return uniqueStable([
...
function getMaterialityMetadata(input: BuildCloseHealthPackageInput): SyntheticAuditMaterialityCompatibility[] {
...
function buildPackageId(input: BuildCloseHealthPackageInput): string {
  return `synthetic-close-health-package:${stableSnapshotHash({
    ...
    ...getCloseHealthObservationIds(input),
...
function getForwardCompatibilityWarnings(input: BuildCloseHealthPackageInput): string[] {
  return [
...
function validateInput(input: BuildCloseHealthPackageInput): string[] {
...
export function buildCloseHealthPackage(input: BuildCloseHealthPackageInput): BuildCloseHealthPackageResult {
  const fatalWarnings = validateInput(input);
  if (fatalWarnings.length > 0 || !input.auditContract) {
    return {
      closeHealthPackage: null,
      skipped: true,
