...
export interface BuildSageIntacctConnectorInput extends Partial<IntegrationBaseContract> {
  connectorId?: string;
  connectorKind?: "erp";
  connectorService?: "sage_intacct";
  firmTenantId?: string;
  ...
  phase39SageIntacctAdapterReferenceId?: string;
  ...
  nativeMultiEntitySupported?: true;
  multiEntityScopedToTenant?: true;
  entityScopeReferenceIds?: string[];
  canonicalMappingReference?: string;
  postsAsDraftNeverAutoApproves?: true;
  sageIntacctConnectorComplete?: boolean;
}

export interface SyntheticSageIntacctConnector extends IntegrationBaseContract {
  sageIntacctConnectorId: string;
  sageIntacctConnectorKey: string;
  connectorId: string;
  connectorKind: "erp";
  connectorService: "sage_intacct";
  firmTenantId: string;
...
export interface BuildSageIntacctConnectorResult {
  sageIntacctConnector: SyntheticSageIntacctConnector | null;
  skipped: boolean;
  warnings: string[];
}
...
function collectMissingRequiredIdentifiers(input: BuildSageIntacctConnectorInput): string[] {
  ...
  if (!hasValue(input.phase39SageIntacctAdapterReferenceId)) {
    missing.push("phase39SageIntacctAdapterReferenceId");
  }

  if (getInputArray(input.entityScopeReferenceIds).length === 0) {
    missing.push("entityScopeReferenceIds");
  }
...
function buildSageIntacctConnectorKey(input: BuildSageIntacctConnectorInput): string {
  return stableSnapshotHash({
    ...
    phase39SageIntacctAdapterReferenceId: input.phase39SageIntacctAdapterReferenceId ?? "",
    entityScopeReferenceIds: getInputArray(input.entityScopeReferenceIds),
    canonicalMappingReference: input.canonicalMappingReference ?? "",
    boundPhase40SnapshotHash: input.boundPhase40SnapshotHash ?? "",
  });
}

function buildSageIntacctConnectorId(input: BuildSageIntacctConnectorInput): string {
  return `synthetic-sage-intacct-connector:${stableSnapshotHash({
    sageIntacctConnectorKey: buildSageIntacctConnectorKey(input),
    artifactType: "SyntheticSageIntacctConnector",
  })}`;
}

function buildDerivationHash(input: BuildSageIntacctConnectorInput): string {
  return stableSnapshotHash({
    connectorId: input.connectorId ?? "",
    connectorKind: "erp",
    connectorService: "sage_intacct",
...
function getWarnings(input: BuildSageIntacctConnectorInput): string[] {
  return [
    ...getInputArray(input.warnings),
    "Sage Intacct connector is metadata only; no live Sage API call, OAuth flow, or data pull is performed",
    "live Sage Intacct multi-entity scoping and sign-aware classification must be validated in the live-execution pass",
  ];
}

export function buildSageIntacctConnector(
  input: BuildSageIntacctConnectorInput,
): BuildSageIntacctConnectorResult {
  const missingRequiredIdentifiers = collectMissingRequiredIdentifiers(input);

  if (missingRequiredIdentifiers.length > 0) {
    return {
      sageIntacctConnector: null,
      skipped: true,
      warnings: [
        ...getInputArray(input.warnings),
  ...
  const requiredPhase39SageIntacctAdapterReferenceId = input.phase39SageIntacctAdapterReferenceId as string;
  const requiredCanonicalMappingReference = input.canonicalMappingReference as string;

  const sageIntacctConnector: SyntheticSageIntacctConnector = {
    ...sharedBase,
    sageIntacctConnectorId: buildSageIntacctConnectorId(input),
    sageIntacctConnectorKey: buildSageIntacctConnectorKey(input),
    ...
    phase39SageIntacctAdapterReferenceId: requiredPhase39SageIntacctAdapterReferenceId,
    reusesPhase39AdapterContracts: true,
    signAwareClassificationRequired: true,
    negativeCashReclassToLiabilityRequired: true,
    contraAccountSignAwareHandlingRequired: true,
    ...
    postsAsDraftNeverAutoApproves: true,
    sageIntacctConnectorComplete: input.sageIntacctConnectorComplete === true,
  ...
  return {
    ...
    skipped: false,
    warnings: sageIntacctConnector.warnings,
  };
}
