...
export type DynamicsConnectorVariant = "business_central" | "finance_and_operations";

export interface BuildDynamicsConnectorInput extends Partial<IntegrationBaseContract> {
  connectorId?: string;
  connectorKind?: "erp";
  connectorService?: "microsoft_dynamics";
  dynamicsVariant?: DynamicsConnectorVariant;
  firmTenantId?: string;
  ...
  phase39DynamicsAdapterReferenceId?: string;
  reusesPhase39AdapterContracts?: true;
  variantSpecificMappingRequired?: true;
  businessCentralAndFinanceOpsHandledDistinctly?: true;
  ...
  nativeMultiLegalEntitySupported?: true;
  multiLegalEntityScopedToTenant?: true;
  legalEntityScopeReferenceIds?: string[];
  canonicalMappingReference?: string;
  postsAsDraftNeverAutoApproves?: true;
  dynamicsConnectorComplete?: boolean;
}

export interface SyntheticDynamicsConnector extends IntegrationBaseContract {
  dynamicsConnectorId: string;
  dynamicsConnectorKey: string;
  connectorId: string;
  connectorKind: "erp";
  connectorService: "microsoft_dynamics";
  dynamicsVariant: DynamicsConnectorVariant;
...
export interface BuildDynamicsConnectorResult {
  dynamicsConnector: SyntheticDynamicsConnector | null;
  skipped: boolean;
  warnings: string[];
}
...
function collectMissingRequiredIdentifiers(input: BuildDynamicsConnectorInput): string[] {
  ...
  if (!hasValue(input.phase39DynamicsAdapterReferenceId)) {
    missing.push("phase39DynamicsAdapterReferenceId");
  }

  if (getInputArray(input.legalEntityScopeReferenceIds).length === 0) {
    missing.push("legalEntityScopeReferenceIds");
  }
...
function buildDynamicsConnectorKey(input: BuildDynamicsConnectorInput): string {
  return stableSnapshotHash({
    ...
    phase39DynamicsAdapterReferenceId: input.phase39DynamicsAdapterReferenceId ?? "",
    legalEntityScopeReferenceIds: getInputArray(input.legalEntityScopeReferenceIds),
    canonicalMappingReference: input.canonicalMappingReference ?? "",
    boundPhase40SnapshotHash: input.boundPhase40SnapshotHash ?? "",
  });
}

function buildDynamicsConnectorId(input: BuildDynamicsConnectorInput): string {
  return `synthetic-dynamics-connector:${stableSnapshotHash({
    dynamicsConnectorKey: buildDynamicsConnectorKey(input),
    artifactType: "SyntheticDynamicsConnector",
  })}`;
}

function buildDerivationHash(input: BuildDynamicsConnectorInput): string {
  return stableSnapshotHash({
    connectorId: input.connectorId ?? "",
    connectorKind: "erp",
    connectorService: "microsoft_dynamics",
...
function getWarnings(input: BuildDynamicsConnectorInput): string[] {
  return [
    ...getInputArray(input.warnings),
    "Dynamics connector is metadata only; no live Dynamics API call, OAuth flow, or data pull is performed",
    "live Dynamics variant handling, legal-entity scoping, and sign-aware classification must be validated against Business Central and Finance & Operations sandboxes",
  ];
}

export function buildDynamicsConnector(input: BuildDynamicsConnectorInput): BuildDynamicsConnectorResult {
  const missingRequiredIdentifiers = collectMissingRequiredIdentifiers(input);

  if (missingRequiredIdentifiers.length > 0) {
    return {
      dynamicsConnector: null,
      skipped: true,
      warnings: [
        ...getInputArray(input.warnings),
  ...
  const requiredDynamicsVariant = input.dynamicsVariant as DynamicsConnectorVariant;
  ...
  const requiredPhase39DynamicsAdapterReferenceId = input.phase39DynamicsAdapterReferenceId as string;
  const requiredCanonicalMappingReference = input.canonicalMappingReference as string;

  const dynamicsConnector: SyntheticDynamicsConnector = {
    ...sharedBase,
    dynamicsConnectorId: buildDynamicsConnectorId(input),
    dynamicsConnectorKey: buildDynamicsConnectorKey(input),
    connectorId: requiredConnectorId,
    connectorKind: "erp",
    connectorService: "microsoft_dynamics",
    dynamicsVariant: requiredDynamicsVariant,
    ...
    phase39DynamicsAdapterReferenceId: requiredPhase39DynamicsAdapterReferenceId,
    reusesPhase39AdapterContracts: true,
    variantSpecificMappingRequired: true,
    businessCentralAndFinanceOpsHandledDistinctly: true,
    signAwareClassificationRequired: true,
    ...
    postsAsDraftNeverAutoApproves: true,
    dynamicsConnectorComplete: input.dynamicsConnectorComplete === true,
  ...
  return {
    dynamicsConnector,
    skipped: false,
    warnings: dynamicsConnector.warnings,
  };
}
