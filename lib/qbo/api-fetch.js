...
import { extractNetworkErrorDiagnostics } from "../network/transport-diagnostics";
...
export async function qboApiFetch(url, options) {
  ...
  try {
    ...
    const transport = extractNetworkErrorDiagnostics(networkErr);
    // Safe diagnostics only: no Authorization, realm, query, or proxy secrets.
    console.error("[qbo-api] network error", {
      method,
      url_path,
      elapsed_ms,
      err: transport.message,
      err_name: transport.name,
      err_code: transport.code ?? transport.deepest?.code ?? null,
      err_errno: transport.errno ?? transport.deepest?.errno ?? null,
      err_syscall: transport.syscall ?? transport.deepest?.syscall ?? null,
      err_hostname: transport.hostname ?? transport.deepest?.hostname ?? null,
      err_port: transport.port ?? transport.deepest?.port ?? null,
      err_cause: transport.deepest
        ? {
            name: transport.deepest.name,
            message: transport.deepest.message,
            code: transport.deepest.code ?? null,
            errno: transport.deepest.errno ?? null,
            syscall: transport.deepest.syscall ?? null,
            hostname: transport.deepest.hostname ?? null,
            port: transport.deepest.port ?? null,
          }
        : null,
      err_cause_chain: transport.causeChain.map((frame) => ({
        name: frame.name,
    ...
    if (throwOnError) {
      throw new QboApiError({
        message: `QBO API network error: ${transport.message || "unknown"}`,
        status: 0,
        intuit_tid: null,
        url,
        payload: null,
  ...
  if (response.status >= 400 && options?.context?.userId) {
    const qboFaultCode = json?.Fault?.Error?.[0]?.code || null;
    const qboFaultMessage = json?.Fault?.Error?.[0]?.Message || null;
    void autoFileTicket({
      userId: options.context.userId,
      source: "qbo",
      ...
      qboFaultCode,
      qboFaultMessage,
      realmId: options.context.realmId || null,
      intuit_tid,
      endpoint: url_path,
      method: (options.method || "GET").toUpperCase(),
      workflowContext: options.context.workflowContext || null,
    }).catch(() => {});
  }
