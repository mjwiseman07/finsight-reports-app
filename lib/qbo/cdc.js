...
//   - Iterates every accounting_connections row where provider='quickbooks'
//     and status='connected'.
//   - For each realm, calls QBO's /cdc endpoint per entity and upserts
//     synthetic reconciliation rows into qbo_webhook_events.
...
//
// Non-goals:
//   - We do NOT process the resulting rows here. dispatchWebhookEvent already
//     runs against every qbo_webhook_events insert via a Supabase trigger?
//     NO — currently only the /webhook route calls dispatchWebhookEvent. For
//     CDC-inserted rows we intentionally do not call the handler; the handler
//     stubs today just log an ack, and future Phase D handlers will pick up
//     CDC rows via the same fetch_pending queue.
...
// Global execution budget for one cron run.
const RUN_BUDGET_MS = 4 * 60 * 1000; // 4 minutes; Vercel maxDuration is 5.
...
export async function reconcileRealm(connectionRow, runId, deadline) {
  ...
  // Update run row (always — including token-refresh failures).
  await admin
    .from("qbo_cdc_runs")
    .update({
...
/**
 * Top-level entry: iterate every active QBO connection and reconcile.
 */
export async function runCdcReconciliation() {
  ...
  const { data: connections, error } = await admin
    .from("accounting_connections")
    .select("id, tenant_or_realm_id, access_token, refresh_token, token_expires_at")
  ...
  for (const conn of connections || []) {
    if (Date.now() > deadline) {
      console.warn("[qbo-cdc] run budget exhausted; deferring remaining realms", {
        remaining: (connections?.length || 0) - summaries.length,
      });
      break;
    }
