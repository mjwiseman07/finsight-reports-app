...
import { autoFileTicket } from "../support/auto-file";
import { supabaseAdmin } from "../supabase";

async function resolveUserIdForRealm(realmId) {
  if (!supabaseAdmin || !realmId) return null;
...
function dispatchCdcAutoFile(userId, realmId, err) {
  if (!userId) return;
  void autoFileTicket({
    userId,
    source: "cdc",
    errorMessage: err?.message || "cdc run failed",
    errorName: err?.name,
    realmId,
    endpoint: "/cdc",
  }).catch(() => {});
...
async function ensureFreshAccessToken(connectionRow) {
  ...
  if (!connectionRow.refresh_token) {
    console.warn("[qbo-cdc] connection missing refresh_token; skipping", {
      hasRealm: Boolean(connectionRow.tenant_or_realm_id),
    });
    return null;
  }

  const concurrencyToken =
    connectionRow.updated_at != null ? String(connectionRow.updated_at) : "";
  const userId = connectionRow.user_id != null ? String(connectionRow.user_id) : "";
  const realmId =
    connectionRow.tenant_or_realm_id != null
      ? String(connectionRow.tenant_or_realm_id)
      : "";
  if (!concurrencyToken || !userId || !realmId || !connectionRow.id) {
    console.warn("[qbo-cdc] connection missing concurrency binding; skipping refresh");
    return null;
  }
  ...
  const originalRefreshToken = connectionRow.refresh_token;
  ...
  try {
    const resp = await fetch("https://oauth.platform.intuit.com/oauth2/v1/tokens/bearer", {
      ...
      body: `grant_type=refresh_token&refresh_token=${encodeURIComponent(originalRefreshToken)}`,
    });
    ...
    const newRefresh = body.refresh_token || originalRefreshToken;
    ...
    try {
      await persistRefreshedQboCredentialsConditional(
        admin,
        {
          connectionId: String(connectionRow.id),
          userId,
      ...
      if (err instanceof QboCredentialCasError) {
        console.warn("[qbo-cdc] credential persist concurrency conflict", {
          code: err.code,
        });
        // Discard returned credentials; do not retry with stale refresh result.
        return null;
      }
      throw err;
    }
  ...
  try {
    try {
      ...
      if (!accessToken) {
        ...
        const userId = connectionRow.user_id || (await resolveUserIdForRealm(realmId));
        dispatchCdcAutoFile(userId, realmId, {
        ...
        for (let i = 0; i < CDC_ENTITIES.length; i += CHUNK_SIZE) {
          ...
          try {
            const { ok, status, json, intuit_tid } = await qboApiFetch(url, {
              accessToken,
              method: "GET",
              context: connectionRow.user_id
                ? { userId: connectionRow.user_id, realmId }
                : undefined,
            });
            ...
            const userId = connectionRow.user_id || (await resolveUserIdForRealm(realmId));
            dispatchCdcAutoFile(userId, realmId, err);
...
export async function runCdcReconciliation() {
  ...
  try {
    const { data: connections, error } = await admin
      ...
      .select("id, user_id, tenant_or_realm_id, access_token, refresh_token, token_expires_at, updated_at")
