...
function toAdapterConnectionShape(row) {
  if (!row) return null;
  const realmId =
    row.tenant_or_realm_id ||
    String(row.external_entity_id || "").replace(/^qbo:/, "") ||
    row.realm_id ||
    null;
  return {
    id: row.id,
...
export class QuickBooksAdapter extends ERPBaseAdapter {
  ...
  async refreshToken(connection) {
    ...
    const { data, error } = await supabaseAdmin
      ...
      .select(
        "id, user_id, access_token, refresh_token, token_expires_at, tenant_or_realm_id, external_entity_id, status",
      )
      .single();

    if (error) throw error;
    return toAdapterConnectionShape(data);
  }
  ...
  async getConnection() {
    ...
    const { data, error } = await supabaseAdmin
      .from(CANONICAL_CONNECTION_TABLE)
      .select(
        "id, user_id, access_token, refresh_token, token_expires_at, tenant_or_realm_id, external_entity_id, status, superseded_by_connection_id, credentials_cleared_at",
      )
      .eq("user_id", this.userId)
      .eq("provider", PLATFORM)
      .eq("status", "connected")
  ...
  async getAuthenticatedClientFromAccountingConnection(connection) {
    const realmId = connection.tenant_or_realm_id || String(connection.external_entity_id || "").replace(/^qbo:/, "");
