...
export class QuickBooksAdapter extends ERPBaseAdapter {
  ...
  async saveConnection({ realmId, token }) {
    // Phase Q7 deviation: this method writes erp_connections / quickbooks_connections
    // only — it does NOT call getCompanyProfile or persist to accounting_connections.
    // Edition + subscription status are captured on the lead OAuth path
    // (saveLeadQuickBooksAccountingConnection) and retrofitted by health-checker.
    // Do not require qbo_edition fields on this erp_connections payload.
    if (!supabaseAdmin) {
  ...
  async getAuthenticatedClientFromAccountingConnection(connection) {
    ...
    if (isExpiredOrExpiring(tokenExpiry)) {
      ...
      const concurrencyToken =
        connection.updated_at != null ? String(connection.updated_at) : "";
      const userId =
        connection.user_id != null
          ...
          : this.userId
            ? String(this.userId)
            : "";
      if (!concurrencyToken || !userId || !connection.id) {
        throw new Error("QuickBooks accounting connection concurrency token is missing");
      }
      ...
      const token = tokenResponse.payload;
      const nextAccess = token.access_token;
      const nextRefresh = token.refresh_token || originalRefreshToken;
      ...
      const { persistRefreshedQboCredentialsConditional, QboCredentialCasError } = await import(
        "../integrations/accounting/canonical-qbo-credential-cas"
      );
      try {
        await persistRefreshedQboCredentialsConditional(
          supabaseAdmin,
          {
            ...
            concurrencyToken,
            expectedRefreshToken: originalRefreshToken,
