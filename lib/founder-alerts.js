 ...
 */
export async function sendFounderAlert({ refundRequestId, subject, body, context = {}, subjectLine }) {
  ...
  const emailSubject = subjectLine || `[Advisacor Refunds] ${subject}`;
...
export async function alertPathBReviewNeeded({
  refundRequest,
  subscription,
  customerMessage,
  sentimentFlags,
  ...
  const body = [
    ...
    `  https://advisacor.com/admin/refunds?highlight=${refundRequest.id}`,
    ``,
  ...
  await sendFounderAlert({
    ...
    subjectLine: `[Refund Queue] New Path B request — ${companyName || refundRequest.requester_email}`,
    body,
