...
describe("v1.4 tier launch_status defaults", () => {
  ...
  it("review_assist_pro is LIVE (Track 4 Option C)", () => {
    expect(TIERS.review_assist_pro.launch_status).toBe(LAUNCH_STATUS.LIVE);
  });

  it("remaining v1.4 add-ons start as testing", () => {
    const testingKeys = [
      "ra_cashapp_addon",
      "ra_je_write_addon",
...
describe("getLiveTiers", () => {
  ...
  it("includes promoted review_assist_pro and excludes still-testing review_assist", () => {
    const live = getLiveTiers();
    expect(live).toContain("review_assist_pro");
...
describe("getTestingTiers", () => {
  it("returns remaining v1.4 testing tiers (RA Pro promoted to live)", () => {
    const testing = getTestingTiers();
    expect(testing.length).toBeGreaterThanOrEqual(9);
    expect(testing).toContain("review_assist");
    expect(testing).not.toContain("review_assist_pro");
...
describe("getSuperAdminPackageGroups", () => {
  ...
  it("v14Live contains review_assist_pro after Track 4 promotion", () => {
    const groups = getSuperAdminPackageGroups();
    expect(groups.v14Live).toContain("review_assist_pro");
  });

  it("v14Testing contains remaining v1.4 tiers still under QA", () => {
    const groups = getSuperAdminPackageGroups();
    expect(groups.v14Testing).toContain("review_assist");
    expect(groups.v14Testing).not.toContain("review_assist_pro");
