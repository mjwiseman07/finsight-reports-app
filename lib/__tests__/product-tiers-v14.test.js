...
describe('product-tiers v1.4 catalog', () => {
  ...
  describe('v1.4 launch_status field presence (Block 2)', () => {
    it('every v1.4 tier has a launch_status field', () => {
      const requiredV14Keys = [
        'review_assist',
        'ra_cashapp_addon',
        'ra_je_write_addon',
        'review_assist_pro',
      ...
      for (const key of requiredV14Keys) {
        expect(TIERS[key].launch_status, `${key} missing launch_status`).toBeDefined();
      }
    });

    it('solo_bookkeeper is archived via launch_status', () => {
      expect(TIERS.solo_bookkeeper.launch_status).toBe('archived');
    });
  });
});
