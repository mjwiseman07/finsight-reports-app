...
export const TIERS = {
  ...
  review_assist_pro: {
    ...
    entitlements: {
      ...
      max_verticals: 15,
      audit_ready_upgrade_available: true,
      pulse_intelligence: true,
      organizational_memory: true,
      predictive_alerts: true,
      weekly_briefings: true,
...
// ============================================================
// v1.5 — Audit Ready add-on SKUs (Track 4 Option C)
// RA Pro base ($199/mo) covers scope <= 2 entities / <= 150 PBC requests.
// Audit Ready add-ons are true upsells for engagements exceeding that scope.
// Small + Standard tiers ARCHIVED in Track 4 (Aug 2026) — dominated by RA Pro base.
// ============================================================
...
/**
 * RA Pro base tier scope caps.
 * Engagements exceeding these limits must attach an Audit Ready add-on.
 * See Track_C_Phase_1_Tier_Spec_v1_5.md §7A.2 (Track 4 Option C revision).
 */
export const REVIEW_ASSIST_PRO_BASE_LIMITS = Object.freeze({
  max_entities: 2, // keep in sync with lib/review-assist-pro/limits.ts RA_PRO_INCLUDED_CLIENT_COMPANIES
  max_pbc_requests: 150,
  max_auditor_users: 5, // firm seats — keep in sync with RA_PRO_FIRM_SEATS
});

/**
 * Deterministic tier assignment given engagement facts.
 * Returns null when RA Pro base scope suffices (no add-on needed).
 * See Track_C_Phase_1_Tier_Spec_v1_5.md §7A.2 (Track 4 Option C revision).
 */
export function assignAuditReadyTier(input) {
  ...
  if (
    entity_count > REVIEW_ASSIST_PRO_BASE_LIMITS.max_entities ||
    pbc_request_count > REVIEW_ASSIST_PRO_BASE_LIMITS.max_pbc_requests
  ) {
    return "complex";
  }
  return null; // RA Pro base scope covers this engagement
}
