 ...
 *
 * Ordering: strip hash first, then setSession — a failed setSession must not leave
 * secrets in the URL. Query string (e.g. ?resumeConnect=…) is preserved.
 */
export async function consumeAuthHashFromUrl(supabase: AuthClient): Promise<{
  ...
  // Same cookie write as /signin and MFA challenge — used by reviewer/MFA server paths.
