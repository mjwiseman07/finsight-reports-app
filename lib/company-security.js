...
export async function resolveCompanyMembership({ userId, companyId, requiredPermission }) {
  ...
  if (error || !membership) {
    await auditSecurityEvent({
      eventType: "company_access_denied",
      actorUserId: userId,
      companyId,
      resourceType: "company",
      resourceId: companyId,
      metadata: { reason: "missing_active_membership" },
    });
  ...
  if (requiredPermission && !roleHasPermission(membership.role, requiredPermission)) {
    await auditSecurityEvent({
      eventType: "company_permission_denied",
      actorUserId: userId,
      companyId,
      resourceType: "company",
      resourceId: companyId,
      metadata: { role: membership.role, required_permission: requiredPermission },
    });
