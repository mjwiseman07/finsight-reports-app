# Advisacor Phase Atlas — v2.15 (authoritative for LOCK-G7)

> **AUTHORITATIVE LOCK DOCUMENT — G7 disclosure substrate frozen at LOCK-G7 tag.**
> **Supersedes v2.14 workspace atlas for post-G7 sequencing. Historical anchor: `Advisacor_Phase_Atlas_v1_2_8_authoritative_for_lock_42_7_recon2.md` (LOCK-42.7).**
...
**Product:** Advisacor
**Atlas Owner:** Cursor (G7 LOCK synthesis)
**Generated:** 2026-06-18 v2.15 — LOCK-G7
**Repository:** `C:\Users\mattj\finsight-reports`
**LOCK anchor:** `b705706` (G7-C7b) → C7c commit + `LOCK-G7` tag

## G7 LOCKED — DISCLOSURE SUBSTRATE IMMUTABLE (LOCK-G7)

`atlasVersion: 2.15` · `g7Status: LOCKED` · `g7LockTag: LOCK-G7` · `registerState: 0/91/0/110` · `registerTotal: 201`  
`schemaVersion: 1.2.0` · `cascadeRunnerVersion: 1.2.0` · `cascadeRunnerStages: 29` · `crossoverValidators: 7`  
`realFilingsExtracted: 45` · `verifierMeta: 26/26` · `verifierMain: 435/435` · `emitterMatrixRows: 453`  
`frameworkNonComingling: enforced` · `verticals: 9` · `frameworks: US GAAP + IFRS`  
**Sequencing:** G10 mock-co regression → F-series (blocked on G10 + Phase 35 + 42.5C)

### G7 sub-commit seal (complete)

| Block | Commits | Status |
|---|---|---|
| C0–C5 | Validators scaffold | ✅ SEALED |
| C5.5 | Hardening (a–f) | ✅ SEALED |
| C6 | Triage | ✅ SEALED |
| C6.1 | Surgical re-triage | ✅ SEALED |
| C6.2 | Reclassifications | ✅ SEALED |
| C7a | -1 through -15 (incl. -12/-13/-14 no-ops) | ✅ SEALED |
| C7b | Crossover validators + cascade 23–29 + schema v1.2.0 | ✅ SEALED (`b705706`) |
| C7c | Atlas v2.15 + Compliance v1.7 + LOCK-G7 tag | ✅ THIS COMMIT |

### Register at LOCK (201 entries)

| Triage | Count |
|---|---:|
| fix-now | 0 |
| document-limitation | 91 |
| defer-to-future | 0 |
| satisfied | 110 |

### Vertical lane status (9 verticals — all LOCKED)

| Vertical | US GAAP lane | IFRS lane | Emitters (US/IFRS) | Satisfied | Doc-lim |
|---|---|---|---|---:|---:|
| Healthcare (hc) | LOCKED | LOCKED | 5 / 4 | 15 | 10 |
| Manufacturing (mfg) | LOCKED | LOCKED | 2 / 1 | 9 | 16 |
| Retail (rtl) | LOCKED | LOCKED | 3 / 4 | 7 | 10 |
| Fund Accounting (fa) | LOCKED | LOCKED | 8 / 7 | 20 | 8 |
| GovCon (gc) | LOCKED | LOCKED | 5 / 2 | 8 | 14 |
| Construction (con) | LOCKED | LOCKED | 7 / 7 | 19 | 10 |
| Professional Services (ps) | LOCKED | LOCKED | 3 / 2 | 11 | 6 |
| SaaS (saas) | LOCKED | LOCKED | 7 / 7 | 16 | 5 |
| Nonprofit (npo) | LOCKED | IPSAS LOCKED | 3 / 2 | 5 | 12 |

Framework non-comingling enforced at lane-gate level for all 9 verticals. Cross-cutting framework-switch fixtures present per vertical lane pair where both lanes exist.

### Cascade stage registry (29 stages)

**Stages 1–22 (pre-C7b):** tsc, verifier, 9× K-V verticals, control, doctrine, audit, lint (advisory), integration-c1..c7.

**Stages 23–29 (C7b crossover):**

| Stage | Name | Halt on failure |
|---:|---|---|
| 23 | framework_consistency | YES |
| 24 | emitter_path_integrity | YES |
| 25 | crossover_footing | YES |
| 26 | lessor_gap_surveillance | NO (advisory) |
| 27 | register_classification | YES |
| 28 | timestamp_drift_normalization | NO (advisory) |
| 29 | collapse_step_documentation | YES |

### C7b backlog — ALL CLOSED

| Item | Finding |
|---|---|
| B1 | NPO 5-gaps-1-emitter — legitimate 1:N (2 assertion hooks in `functionalExpenseAllocation.ts`) |
| B2 | Timestamp drift — `.gitattributes` smudge + `stripGeneratedAt` runtime |
| B3 | Collapse-step — `collapse_step` schema field + `collapseStepValidator` |
| B4 | Verifier matrix policy — matrix wiring + unit tests both required for new emitters |
| B5 | Lessor surveillance — active cascade stage 26; no lessor gaps detected |
| B6 | GAP-0174/0177/0180 — US GAAP `rtl/channel-disaggregation` doc-lim confirmed |

### Architectural patterns codified (G7)

- Per-gap `emitter_path` precision (schema v1.2.0)
- Collapse-step closure (`reclassified_in` + `closed_in` same commit)
- `FrameworkViolationError` / `CrossoverViolationError` hierarchy
- `framework_substitute_note` pattern (IFRS/US GAAP asymmetry)
- `framework_violation_handling` pattern (invalid framework rejection)
- Forbidden-substring grep per-lane
- Cross-cutting framework-switch fixtures (one per vertical lane pair)
- Single-emitter correctness audit (B1)
- 5-gate pre+post protocol; `git reset --hard HEAD` on red gate

### Crossover validators (`lib/router/crossover/`)

1. `frameworkConsistencyValidator` — entity-level framework comingling (fatal)
2. `emitterPathValidator` — per-gap emitter_path + assertion hook (fatal)
3. `crossoverFootingValidator` — cross-disclosure footing (fatal)
4. `lessorGapSurveillance` — B5 active monitoring (warn-only)
5. `registerClassificationValidator` — triage/closed_in consistency (fatal)
6. `timestampDriftValidator` — generatedAt normalization (warn-only)
7. `collapseStepValidator` — collapse-step pattern docs (fatal)

### What's next (post-G7)

- **G10:** mock-co regression (25 mock-cos × 9 verticals × 2 frameworks)
- **F-series** (firm-wedge surfaces): blocked on G10 + Phase 35 + 42.5C deeper audit
- **IP counsel engagement:** queued

---

## G2 CLOSED — NO STUBS FOUND (LOCK-G2-CLOSED @ `608bccc`)

`atlasVersion: 2.14` · `g2Status: CLOSED-NO-STUBS-FOUND` · `g2ClosureSha: 608bccc` · `g2DecisionEvidenceSha: 2b19a58` · `g2DecisionEvidencePath: docs/decisions/Phase_G2_0_Soft_Review_Findings.md`  
...
`integrationHarnessInstalled: true` · `integrationScenarioCount: 18` · `cascadeRunnerStages: 29` · `cascadeReportSchemaVersion: 1.2.0`  
...
| v2.14 | 2026-06-27 | Cursor | **LOCK-G2-CLOSED** (`608bccc`) — G2 formally closed; 0 hard stubs; soft review 13/13 (12 INTENTIONAL-PATTERN, 1 FALSE-POSITIVE); evidence `docs/decisions/Phase_G2_0_Soft_Review_Findings.md`; `g2ClosureSha: 608bccc`; `g2Status: CLOSED-NO-STUBS-FOUND`; `cascadeStatus` gains STUB-CLEAN suffix; sequencing G7 → G10. |
...
*End of Advisacor_Phase_Atlas_v2_15_authoritative_for_LOCK_G7.md (v2.15 — G7 LOCKED; LOCK-G7)*
