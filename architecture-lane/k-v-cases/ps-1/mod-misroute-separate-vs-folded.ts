...
export function runCases() {
  const separate = routeModification(PROF_SERVICES_KV_CTX, { separateContract: true, remainingDistinct: false });
  const folded = routeModification(PROF_SERVICES_KV_CTX, { separateContract: false, remainingDistinct: false });
  return [
    {
      id: "KV-MOD-1",
